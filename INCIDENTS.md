...
### 증상
- 사용자: "궁합 누르면 모달이 켜짐과 동시에 궁합페이지로 넘어간다"
- 사용자: "모달이 뜨고 취소 누르면 홈에서 머물러 있어야하는데 자꾸 궁합분석으로 넘어가"
- 궁합 진입 시 이전 결과 리스트 모달과 카테고리 선택 화면이 함께 보임 →
  버튼을 누르지 않았는데 이미 화면이 넘어간 것처럼 느껴짐
- 모달의 취소·X·배경을 눌러도 홈이 아니라 궁합 카테고리 화면이 드러남

### 영향 범위
- 궁합(GunghapPage) — landing 화면을 모달 UX 로 전환한 직후부터 매 진입 시 발생
- 같은 세션에서 3회에 걸쳐 부분 수정했으나 매번 "여전히 그대로" 신고 반복

### 진단 과정 — 잘못된 경로도 기록 (재발 시 trace 회피용)
1. ❌ **카테고리 카드만 숨기면 된다** (fa280e3) — `showArchiveList` 일 때
   카테고리 카드를 `null` 처리. 하지만 헤더("궁합 분석")·스텝 인디케이터(1·2·3)는
   조건 없이 항상 렌더 → 모달 뒤로 페이지 chrome 이 그대로 비침 → "그대로"
2. ❌ **빌드 깨짐으로 배포 누락** 가설 — 다른 세션의 TodaySectionVisuals 빌드
   에러로 fa280e3 배포가 막혔다고 추정. `vercel inspect` 로 최신 prod 배포가
   fa280e3 의 후손 커밋(463d1ea)임을 확인 → 배포는 정상, 코드 문제로 결론
3. ✅ **페이지 본체 전체를 게이트** (5ad7801) — `modalGateActive` 도입,
   모달 노출 중엔 헤더·스텝·카테고리 전부 미렌더. 동시 노출은 해결.
   하지만 취소 시 여전히 카테고리 화면이 드러남
4. ✅ **취소의 목적지 문제** (6715907) — 진짜 핵심. 모달이 `/saju/gunghap`
   라우트 *안*에 있어 취소가 "돌아갈 곳"이 없었음
5. ✅ **더블체크 중 회귀 발견** (3c11299) — `modalGateActive` 가 `?jobId`
   결과/진행 화면(step='result')까지 가리는 부작용 발견 → step 가드 추가

### 진짜 원인
**렌더 가시성 버그가 아니라 라우팅·내비게이션 의미 버그였다.**
모달이 목적지 라우트(`/saju/gunghap`) 내부에 렌더되므로, 홈에서 궁합을
누르는 순간 이미 라우팅이 끝난 상태다. 모달의 취소는 단순히
`setShowArchiveList(false)` 만 했고 → 모달만 닫히고 그 자리에 궁합
카테고리 화면이 드러남. "취소 = 직전 화면(홈)으로" 라는 사용자 기대를
충족하려면 명시적 `history.back()` 내비게이션이 필요했다.
수정이 늦어진 이유: 매 시도가 "화면을 숨긴다"는 증상 레이어만 건드렸고,
모달이 목적지 라우트 안에 산다는 구조 자체를 늦게 인지함.

### 해결
- `modalGateActive` — 진입 직후 조회 중 / 모달 노출 중엔 페이지 본체
  (헤더·스텝·카테고리) 전체를 미렌더. `step==='category'` 한정.
- `archiveModalIsEntry` — 진입 모달(true) vs 카테고리에서 재호출한
  모달(false) 구분. 진입 모달의 취소·X·배경 → `window.history.back()`
  으로 홈 복귀, 재호출 모달은 카테고리 화면 복귀.
- 진입 모달 자동표시 조건에 `step==='category' && !urlJobId` 가드 추가 —
  `?jobId` 결과 화면 위에 모달이 덮이지 않도록.

### 재발 방지
- **모달을 목적지 라우트 안에 두면 "취소=뒤로"가 공짜로 되지 않는다.**
  취소·닫기는 명시적 내비게이션(history.back / router.back)을 연결할 것.
- 모달 오버레이를 도입할 때 "모달 뒤 배경에 무엇이 보이는가"를 함께 설계.
  헤더·스텝 등 페이지 chrome 도 가려야 하면 본체 전체를 게이트.
- `?jobId`·`?recordId` 진입 화면에 진입 모달/게이트가 덮이지 않는지
  항상 검증 (2026-05-20 23:55 PeriodFortunePage 사고와 동일 계열).
- 같은 증상 신고가 반복되면 증상 레이어가 아니라 한 단계 위
  (라우팅·상태 소유권) 구조를 의심할 것.

### 관련
- 커밋: `fa280e3`(1차 부분), `5ad7801`(게이트), `6715907`(취소→홈),
  `3c11299`(?jobId 가드)
- 파일: `src/pages/GunghapPage.tsx`
- 선행 사고: 2026-05-20 23:55 (?jobId 모드 모달 덮임) — 동일 계열

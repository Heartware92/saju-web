...
### 신년운세 (`newyear`) — Phase 3 완료
| 항목 | 값 |
|---|---|
| 호출 방식 | 2-pass (1차 5섹션 + 2차 4섹션, 총 9섹션) |
| Retry | 0회 (1차 실패 시 환불, 2차 실패 시 1차만 done) |
| 1차 partial | ✅ markPartial — interpretation_basic 에 1차 5섹션 |
| maxTokens | 1차 5500, 2차 6000 |
| 분량 | ~3000자 |
| 서버가 받는 것 | `sajuResult` + `fortune (PeriodFortune)` + `year` + `userCtx` + `isYearFortune` (서버가 prompt 생성) |
| 페이지 패턴 | 결과 페이지 단독 (`PeriodFortunePage` scope='year') — `/saju/newyear` 와 `/saju/year-fortune` 공용 |
| 캐시 정책 | hit 시 옛 동작 유지, miss 시만 새 잡 (변경 최소화) |
| 추가 컬럼 | engine_result (year·isoDate·categoryLabel·source·seWoon) |

### 토정비결 (`tojeong`) — Phase 4 완료
| 항목 | 값 |
|---|---|
| 호출 방식 | 2-pass |
| Retry | 0회 (옛 4단 폴백 제거, 실패 시 자동 환불) |
| 1차 partial | ✅ markPartial — interpretation_basic 에 1차 본문 |
| maxTokens | 1차 8000, 2차 8500 |
| 분량 | ~4200~5500자 |
| 서버가 받는 것 | `tojeongResult` + `saju?` + `userCtx?` (서버가 pass1/pass2 prompt 생성) |
| 페이지 패턴 | 결과 페이지 단독 (`TojeongResultPage`) |
| 캐시 정책 | hit 시 옛 동작 유지 |
| 추가 컬럼 | engine_result 에 TojeongResult |

### 자미두수 (`zamidusu`) — Phase 4 완료
| 항목 | 값 |
|---|---|
| 호출 방식 | 2-pass (1차 6섹션 + 2차 6섹션, 총 12섹션) |
| Retry | 0회 |
| 1차 partial | ✅ markPartial |
| maxTokens | 1차 8000, 2차 8000 |
| 서버가 받는 것 | `zamidusuResult (chart)` (서버가 generateZamidusuPrompt) |
| 페이지 패턴 | 결과 페이지 단독 (`ZamidusuResultPage`) |
| 캐시 정책 | hit 시 옛 동작 유지 |
| 추가 컬럼 | engine_result 에 ZamidusuResult |
...
### 4.8 handleAnalyze try-finally 의 setLoading(false) 충돌 (입력+결과 한 페이지)
- 입력+결과 같은 페이지 패턴(GunghapPage 같은)에서 흔히 발생
- `try { await createXxxJob(...) } finally { setLoading(false); }` 가 잡 생성 후
  실행되면 로딩 사라짐 → 빈 결과 UI 0.5~1초 번쩍 → useFortuneJob.select() 결과
  도착 → 동기화 useEffect 가 setLoading(true) → 로딩 새로 시작 (jump back)
- **해결 패턴**: `let jobCreated = false;` 플래그 + 잡 생성 직전 `jobCreated = true;`
  + finally 에서 `if (!jobCreated) setLoading(false);` — 잡 생성 후엔 잡 동기화
  useEffect 가 loading 책임 일원화
- INCIDENTS 2026-05-20 23:30 참조

### 4.10 ?jobId 모드에서 findRecentArchive·cacheGate 모달 표시 사고
- 보관함에서 진행 중 잡 클릭 → ?jobId 진입 시 페이지의 cacheGate useEffect 가
  `if (isArchiveMode) return;` 만 가드하고 `urlJobId`·`createdJobId` 가드 누락
- findRecentArchive 가 같은 birth + category 의 옛 archive 찾음 → 모달 노출
- "기존 풀이 보겠습니까?" RestoreReportModal 이 잡 진행 중에 띄워지는 사고
- **해결 패턴**: cacheGate useEffect 시작 부분에 `if (effectiveJobId) return;`
  가드 추가. ?jobId 또는 새 잡 생성된 경우 saju_records 가 단일 source of truth,
  cacheGate 분기 자체 skip. useEffect deps 에도 effectiveJobId 추가 필수
- INCIDENTS 2026-05-20 23:55 참조

### 4.9 AILoadingBar 의 progress 단조 증가 보장 (jump down 차단)
- AILoadingBar mount 시점부터 자연 증가 → progress 18% 까지
- 그 사이 useFortuneJob.select() 결과로 startedAt prop 도착 (서버 markProcessing 시점)
- elapsed = 1~2초 기반 재계산 → progress ~2% → setProgress(2%) → **jump down 18% → 2%**
- **해결 패턴**: AILoadingBar 의 setProgress 호출을 모두 `setProgress(p => Math.max(p, newProgress))`
  로 단조 증가 보장. 단조 증가는 새로고침 시 jump up (큰 elapsed) 은 허용, jump down 만 차단
- INCIDENTS 2026-05-20 23:30 참조
...
| gunghap (궁합) | ✅ Phase 2 완료 | `26face5` → `729332d` → `bb52c5c` → `9c2af28` |
| newyear (신년·연도별) | ✅ Phase 3 완료 | `80c579b` → `180d9e1` |
| tojeong (토정비결) | ✅ Phase 4 완료 | `a85c7e3` |
| zamidusu (자미두수) | ✅ Phase 4 완료 | `a85c7e3` |
| taekil (택일) | ✅ Phase 5 완료 | `83c3dd1` |
| date (지정일, category='period') | ✅ Phase 5 완료 | `83c3dd1` |
| today (실시간) | ✅ Phase 6 완료 | (이번 commit) |
| 더많은운세 (study·children·personality·name·dream) | ✅ Phase 6 완료 | (이번 commit) |
| love·wealth·career·health·people | (비활성 — 옛 B안 폐지) | — |
| 타로 (today·monthly·question) | ✅ Phase 7 완료 — tarot_records 테이블 (039 마이그레이션 + useFortuneJob table 분기) | (이번 commit) |

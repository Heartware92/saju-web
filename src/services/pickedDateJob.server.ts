...
export async function runPickedDateJob(input: RunPickedDateJobInput): Promise<void> {
  ...
  try {
    ...
    const pass1Raw = await callAI(pass1Prompt, PASS1_MAX_TOKENS, { temperature: 0.75, systemPrompt: SPIRIT_SYSTEM_PROMPT });
    ...
    try {
      ...
      const pass2Raw = await callAI(pass2Prompt, PASS2_MAX_TOKENS, { temperature: 0.75, systemPrompt: SPIRIT_SYSTEM_PROMPT });

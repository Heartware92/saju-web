...
export async function loadAdminBundle(): Promise<RawBundle> {
  ...
  // 가입 완료(약관 동의 = user_agreements 행 보유 + 휴대폰 인증 완료) 회원만 집계 대상.
  // 미완성 가입(소셜 로그인/약관까지만 하고 휴대폰 인증 전에 이탈)은 회원이 아니므로 제외.
  ...
  // 휴대폰 인증 완료 판정:
  //   - 소셜(구글·카카오): 우리 전용 키 user_metadata.phone_verified_at 로 판정
  //     (Supabase 기본 phone_verified 는 카카오 재로그인 시 false 로 덮어써져 신뢰 불가)
  //   - 이메일: 가입 폼에서 OTP 인증 후 user_metadata.phone 만 저장(phone_verified_at 미설정)
  const isPhoneVerified = (u: (typeof allUsers)[number]) => {
    const provider = u.app_metadata?.provider;
    const isSocial = !!provider && provider !== 'email';
    return isSocial
      ? !!u.user_metadata?.phone_verified_at
      : !!(u.user_metadata?.phone || u.user_metadata?.phone_verified_at);
  };
  const users = agErr ? allUsers : allUsers.filter((u) => completedIds.has(u.id) && isPhoneVerified(u));

...
★★★ '사주풀이'임을 모든 섹션에서 잊지 말 것 — 누구에게나 똑같이 해줄 수 있는 일반 상식·생활 팁·범용 조언으로 본문을 채우면 전부 사고다. (사고 예: 수면 "따뜻한 물 샤워·잠들기 전 스마트폰 자제·명상 음악·스트레칭" / 건강 "물 자주 마시기·골고루 먹기" / 관계 "진심으로 대화하기·경청" / 운동 "가볍게 몸 풀기" / 격려 "당신답게 빛나는 하루" — 사주 없이도 할 수 있는 말은 모두 금지.) 모든 권장·묘사·조언은 "오늘 일진(천간·지지 오행·십성·합충)이 내 사주(용신·결핍오행·신강약·자극받는 궁)와 만나 '오늘' '나에게' 왜 그런지"의 명리 인과에서 나와야 한다. 같은 행동을 권하더라도 그 앞에 '오늘 왜 나에게'가 명리로 깔려야 비로소 풀이다.
...
★ 자율성 원칙 (가장 중요) — 각 구간에 '활동'을 못박지 말 것. "출근·오전 업무", "점심·오후 회의·외출", "퇴근·저녁 식사" 같은 특정 직업(직장인) 일과를 단정하지 말고, 그 시간대가 하루에서 갖는 의미(예: 아침=막 하루를 여는 시점, 한낮=하루 한복판, 저녁=하루를 닫아가는 시점)만 제시한 뒤, 오늘 일진 ${todayGz.gan}${todayGz.zhi}와 내 사주의 상호작용으로 "그 구간을 어떻게 쓰면 좋은지"를 자유롭게 푼다. 사용자 직업을 알면(현재: ${jobLabel || '미입력'}) 그 일과로 1회 자연 변형, 모르면 특정 직업을 전제하지 않은 장면으로(학생·자영업·주부·프리랜서·무직 누구에게나 맞되, 일반 상식이 아니라 오늘 일진×사주로 구체화) — 직장인 전제 어휘 금지. (아래 구간 설명의 괄호 예시는 '의미' 참고용이지 활동 지정이 아니다.)
...
★ 권장 취침/기상 시각을 억지로 정해 박지 말 것 — "몇 시에 자고 몇 시에 일어나라" 식 고정 룰 금지. 대신 오늘 일진 흐름상 수면·회복이 어떤 결인지(깊게 쉬어야 하는 날인지, 머리가 맑아 늦게까지 깨어 있기 쉬운 날인지 등)를 명리로 판단해 자연스럽게 풀고, 사용자가 본 시점(${slotLabel})을 고려해 그 시점 이후의 회복 흐름으로 잇는다.
★★ 누구에게나 통하는 일반 수면 상식만 늘어놓지 말 것 — "따뜻한 물로 샤워·잠들기 30분 전 스마트폰 자제·명상 음악·가벼운 스트레칭" 같은 범용 수면 위생 팁은 사주풀이가 아니다(이런 보편 조언만으로 채우면 사고). 권장하는 행동 하나하나가 "오늘 일진의 오행(천간 ${todayGz.ganElement}·지지 ${todayGz.zhiElement})·십성·합충(${interTodayStr})이 내 사주(용신 ${yongSinElement}·결핍오행 ${zeroEls.length > 0 ? zeroEls.join('·') : '없음'}·신강약 ${result.strengthStatus})와 만나, 오늘 밤 내 몸·마음이 어떤 상태이기 때문"인지 명리 인과로 연결돼야 한다 — 같은 행동을 권하더라도 '오늘 왜 나에게'가 앞에 깔려야 한다. 예: 화 기운 과한 날→머리·심장이 과열돼 잠이 얕으니 열을 식히는 결로 / 수 부족·신장 약한 날→하체 보온·수분으로 신(腎) 보강 / 일진 충으로 신경 곤두선 날→자극 차단·이완 우선. 직업(${jobLabel || '미입력'})을 알면 취침 패턴에 1회 자연 반영. 구성·강조는 자유.
분량: 240~340자.
...
(2) 식습관 — 그 건강 결에 맞춰 오늘 챙기면 좋은 음식·식재료, 어울리는 맛·색(용신·결핍오행 보강), 피할 음식, 식사 톤(천천히·따뜻하게 등)을 구체적으로. 지금 이후 끼니만(afternoon=저녁·간식, evening=저녁·야식·자기 전 음료, midnight/morning=하루 전체). 지나간 끼니 가이드 금지.
...
★ 오늘 "어떤 운동"이 좋은지 구체 종목 1~2개를 콕 집어 추천한다(운동 종류가 많으니 좁혀줄 것 — 막연한 "가벼운 운동" 금지). 종목은 오늘 일진 천간 ${todayGz.gan}(${todayGz.ganElement})·지지 ${todayGz.zhi}(${todayGz.zhiElement})·십성(${todayGz.tenGodGan}/${todayGz.tenGodZhi})이 내 사주(신강신약 ${result.strengthStatus}·합충 ${interTodayStr})와 어떻게 만나는지로 정한다.
...
첫 줄: 은유 제목.
오늘 합충(${interTodayStr})·흉성 신살(${sinSalBad})에서 비롯되는 실수·함정과 멘탈이 흔들리기 쉬운 지점을 구체 장면으로 짚고, 피하는 법·대처도 함께 담는다(흉으로만 단정하지 말 것). ★ 닫는 문장 형태는 매일 자유 — 매번 같은 "~하라" 명령형으로 끝내지 말 것. 직업(${jobLabel || '일상'}) 관련 함정을 1줄 녹여도 좋다. 구성 자유.
분량: 260~360자.
...
${ctx.customJobState?.trim()
  ...
  : `직업 미입력 — 특정 직업·직장 장면을 만들지 말고, 취미(${primaryHobby})·진입 시간대(${slotLabel})와 오늘 일진×내 사주에서 끌어낸 구체적인 작은 실천을(누구나 할 일반 조언 말고). 첫 줄은 "오늘의 작은 시도" 류.`}

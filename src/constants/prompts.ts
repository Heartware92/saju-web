...
[작성 지침] 7개 섹션 구조 — 각 섹션은 [key] 마커로 시작. 총 본문 1450~1900자. 5달 가치에 맞게 풍부하게.${result.hourUnknown ? '\n※ 시간미상이므로 자녀궁 직접 해석은 제한하고 연·월·일주 + 대운·세운 중심으로 풀이' : ''}

★ 출력 형식 (반드시 준수):
[fortune]
[은유] (한 줄 은유 부제목 — 18자 이내, 계절·자연·빛 이미지. 정통사주와 동일 마커 형식)
(빈 줄)
(본문 200~260자 — 다음 모두 포함, 자연스럽게 한 호흡으로:
 1) 자녀복 경향 한 줄 단정 (다자·소자·만득·귀한 자녀 한 명 중 하나)
 2) 명리 근거 — 자녀성(남자=관성 정관·편관 / 여자=식상 식신·상관) 분포 + 자녀궁(시주) 12운성·공망 여부 2~3가지를 자연스럽게 노출. 십성 용어는 같은 호흡 안에서 일상어 즉시 풀이
 3) 자녀와의 인연 결 2~3문장 — 이른 자녀·늦은 자녀, 자녀와의 정이 깊은 결, 자녀로 인한 기쁨·책임의 방향 중 어떤 결인지 구체적으로
 4) 양면 묘사 1~2문장 (자녀복 풍성=든든함·깊은 책임감 동반 / 인연 박함=정성껏 한 명에 집중하는 깊이 등)
 추상 격언 금지, 구체 인상으로)

[pregnancy]
(본문 140~190자 — 자녀성 분포로 본 임신·출산 체질을 충분히 풀어:
 · 자연 임신 유리 / 시기 중요 / 의료 도움 권장 중 하나로 단정 + 명리 근거 1~2개
 · 임신·출산기에 본인이 보일 모습 1문장 (체력·정서·태교 성향)
 · 양면 묘사 1문장 (자녀복 풍성=양육 부담 양면 / 인연 박함=자유로움 양면))

[temperament]
(본문 130~180자 — ${result.hourUnknown ? '연·월·일주에서 유추한' : '자녀궁(시주) 12운성·지장간으로 본'} 자녀의 타고난 기질을 구체적으로:
 · 기질 단정: 활동적/차분/예술적/학구적 중 하나 + 명리 근거
 · 일상 행동 묘사 2~3가지 (놀이·식사·잠·친구관계 중 골라 구체 장면)
 · 부모 입장에서 가장 인상적으로 느낄 자녀의 모습 1문장)

[parenting]
(본문 170~220자 — 양육 스타일을 라벨 + 본문 형식으로:
 · 본인의 양육 성향: 인성형(보호·교육 중시) vs 식상형(자유·표현 격려) vs 관성형(규율·책임 강조) vs 재성형(현실 감각 키움) 중 단정 + 명리 근거 한 호흡
 · 본인의 강점: 자연스럽게 잘하는 부분 1~2문장 (구체 장면 예시)
 · 보완할 방향: 의식적으로 보강해야 할 양육 태도 1~2문장 (어떤 상황에서 의식해야 하는지)
 · 부모-자녀가 가장 깊어지는 순간 1문장)

[compatibility]
(본문 140~180자 — 일주 ${result.pillars.day.gan}${result.pillars.day.zhi} 기준 자녀와의 띠·오행 합·충:
 · 잘 맞는 결: 띠 또는 오행 2개 + 작용 메커니즘 1~2문장 (어떤 상황에서 시너지가 나는지)
 · 부딪치기 쉬운 결: 띠 또는 오행 1개 + 갈등 해소법 1~2문장 (구체 행동))

[career_hint]
(본문 140~180자 — 자녀의 진로·재능 힌트 (자녀성 결로):
 · 식상 강한 자녀 = 예술·창작·표현
 · 인성 강한 자녀 = 학문·연구·교육
 · 관성 강한 자녀 = 리더십·공직·체계 분야
 · 재성 강한 자녀 = 사업·실리·재무
 · 비겁 강한 자녀 = 독립·도전·운동
 위 중 단정 1개 + **구체 분야명 2~3개** + 자녀 어릴 때 부모가 어떻게 키워주면 좋은지 1~2문장)

[timing]
(본문 200~270자 — 현재 대운 + 올해 세운 자녀운 영향 1~2문장 + 임신·출산 유리 시기 3~4개 (연·월 + 명리 근거 한 줄씩) + 실천 조언 5개를 "- " 불릿:
 - 출산 유리한 계절 (이유 한 줄)
 - 양육 시 주의할 점 (구체 행동)
 - 자녀 교육 방향 (구체 분야·방식)
...
[작성 지침] 9개 섹션 구조 — 각 섹션은 [key] 마커로 시작. 총 본문 1700~2200자. 5달 가치에 맞게 풍부하게.

★ 출력 형식 (반드시 준수):
[daymaster]
[은유] (한 줄 은유 부제목 — 18자 이내, 낯선 자리·본모습 대비 이미지. 정통사주와 동일 마커 형식)
(빈 줄)
(본문 140~190자 — 일주 ${p.day.gan}${p.day.zhi}(60갑자 ${dayTraits?.hanja || ''})의 핵심 기질을 풍부하게 풀어:
 · 핵심 키워드 3개 명시 + 각 키워드의 일상 발현 1문장씩
 · 60갑자 특성 데이터 적극 활용 — 천간·지지 조합이 만드는 결을 자연스럽게 묘사
 · 본인이 일상에서 가장 자기다운 순간 1문장
 십성 용어는 일상어 즉시 풀이)

[gyeokguk]
(본문 150~200자 — 격국(${gyeokguk}) + 성패(${gyeokgukStatus.isSuccessful ? '성격' : '패격'}) + 신강신약(${result.strengthStatus})이 만드는 인생 기조와 행동 패턴:
 · 격국의 본질 1~2문장 (어떤 결의 사람인지)
 · 성패에 따른 발현 — 성격이면 장점 발현 양상 / 패격이면 구조적 갈등 + 보완 방향
 · 신강신약이 더하는 색채 1문장
 · 인생의 큰 흐름·선택 패턴 1문장)

[strengths]
(본문 220~280자 — 십성 배치로 본 구체적 강점을 상황별로 충분히:
 · 직장에서: 어떤 식으로 잘하는지 + 동료가 보는 본인의 모습 1~2문장 (구체 장면)
 · 연애에서: 어떤 결의 매력 + 상대가 본인에게 끌리는 지점 1~2문장 (구체 장면)
 · 친구 관계에서: 어떤 자리를 차지하는지 + 친구가 본인에게 기대는 지점 1~2문장 (구체 장면)
 추상 격언 금지)

[outside_view]
(본문 180~240자 — 외부 시선을 **3각도**로 풍부하게:
 · 친한 사람 시선: "오래 본 사람들은 ~라고 말한다" + 그 인상의 명리 근거 한 호흡
 · 처음 만난 사람 시선: "첫인상에서는 ~ 같다는 평을 받기 쉽다" + 그 인상의 명리 근거 한 호흡 (오해받기 쉬운 지점 포함)
 · 직장·공적 자리 시선: "공식 자리에서는 ~ 자리에 어울려 보인다" + 격국·관성·식상 근거 한 호흡
 마지막 1문장 — 본인이 인지하지 못하는 인상 1가지)

[desire]
(본문 150~200자 — 욕구 vs 두려움 대비를 깊이 있게:
 · 가장 되고 싶어 하는 모습 1~2문장 (명리 근거 — 관성=리더 동경 / 비겁 강세=인정 욕구 / 식상=표현 욕구 / 재성=현실 풍요 등) + 일상에서 그 욕구가 드러나는 장면 1문장
 · 가장 피하고 싶어 하는 모습 1~2문장 (명리 근거 — 식상 결핍=실행 주저 / 인성 과다=완벽주의 강박 / 관성 결핍=무책임해 보일까 두려움 등) + 그 두려움이 행동에 미치는 영향 1문장
 추상 격언 금지, 구체 인상으로)

[shadow]
(본문 170~220자 — 숨은 그림자를 풍부하게:
 · 간여지동·병존이 있다면 그 편향성 (외곬·집요함·치우침) — 어떤 상황에서 나오는지 구체 장면
 · 충이 있다면 내면 갈등 패턴 — 어떤 상황에서 마음이 흔들리는지 구체 묘사
 · 형이 있다면 자기 파괴 패턴 — 어떤 상황에서 자기 발등 찍는지 구체 묘사
 사주에 해당 구조 없으면 약한 십성·과다 십성으로 대체. 각 그림자마다 짧은 자기인식 팁 1줄 포함)

[sinsal]
(본문 170~220자 — 성격 관련 신살이 만드는 개성 + **현대적 재해석**:
 · 도화·홍염: 과거 흉 → 현대 인기·매력·인플루언서 자질
 · 역마: 과거 떠돌이 → 현대 글로벌 활동·이동 자유
 · 양인: 과거 칼·횡사 → 현대 결단력·리더십
 · 괴강: 과거 극단 → 현대 강한 카리스마
 · 화개: 과거 외로움 → 현대 예술·연구·심리 깊이
 · 천을귀인: 결정적 조력자 운 — 위기에 사람이 나타나는 결
 · 문창·학당: 학문·글·표현 강점
 있는 신살을 3개 골라 각 한 호흡씩 풍부하게 — 일상 발현 묘사 포함. 없으면 "특별한 성격 신살 없음 — 신살에 의존하지 않는 균형 잡힌 결" 2문장)

[stress]
(본문 150~200자 — 스트레스 vs 회복을 충분히:
 · 스트레스 패턴 2~3개 — 각각 구체 상황 + 감정 + 행동 묘사 1~2문장씩
 · 회복 패턴 1~2개 — 어떤 환경·관계·활동으로 충전되는지 구체 1~2문장
 · 자기 신호 1문장 (스트레스가 쌓였을 때 몸·습관에 나타나는 신호))

[guide]
(본문 250~330자 — 자기관리 가이드 — "- " 불릿 6개 + 마지막 응원:

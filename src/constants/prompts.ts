...
★ 매일 바뀌어야 한다 — 아이템·장소는 이 사람의 고정 성향(격국·용신·관심분야)이 아니라 "오늘 일진"의 오행·십성에 맞춰 정하라. 오늘 일진 천간 ${todayGz.ganElement}·지지 ${todayGz.zhiElement}(${todayGz.gan}${todayGz.zhi}, 십성 ${todayGz.tenGodGan}/${todayGz.tenGodZhi})의 결을 아이템·장소로 옮긴다.
...
★ 반드시 오늘 일진 ${todayGz.gan}${todayGz.zhi}(십성 ${todayGz.tenGodGan}/${todayGz.tenGodZhi})과 내 사주의 구체적 상호작용 1가지(십성·합충 ${interTodayStr}·용신 ${yongSinElement}·일주 12운성 ${pillars.day.twelveStage} 중)를 근거로 — "오늘 이 사람"만의 결을 담아라. ★ 누구에게나 통할 일반 위로·격언만으로 끝내지 말 것(금지 예: "흔들려도 새 길이 있어요", "당신답게 빛나는 하루 보내세요", "예상치 못한 파동이 있어도 잘 헤쳐나갈 거예요" 같은 사주 무관 범용 문장). 명리 용어는 1개 이내로 가볍게 녹이되, 그날의 구체적 기운이 느껴지게.
분량: 100~180자.

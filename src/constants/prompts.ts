...
■ 모든 ▶ 섹션은 반드시 아래 형식을 따릅니다:

(줄1) 은유 부제목 ← 7~20자, 명사구·체언 종결, 단독 한 줄
(줄2) (빈 줄 또는 바로 본문)
(줄3~) 본문 내용 시작

■ 부제목 금지 사항 (하나라도 위반 시 잘못된 풀이):
✗ 마침표(.)로 끝남 → "가을 바람이 분다." ← 금지
✗ '~다', '~요', '~니다'로 끝남 → "서로를 비춥니다" ← 금지
✗ '두 사람'으로 시작 → "두 사람의 에너지는" ← 금지
✗ 30자 초과 → 너무 긴 문장은 본문이지 부제목이 아님
✗ 숫자로 시작 → "1) 첫째로..." ← 금지
✗ '오행', '천간', '지지' 포함 → 명리 용어는 부제목에 넣지 말 것
✗ '%' 포함 → 데이터는 본문에
...
■ 출력 점검: 모든 ▶ 섹션의 첫 줄이 위 규칙을 만족하는지 확인 후 출력하세요.
부제목이 없거나 형식이 틀린 섹션이 하나라도 있으면 전체 풀이가 실패로 간주됩니다.`;
...
5-1) ★ 질문 답변이 입력된 경우 — 위 [질문 답변 활용 원칙]의 매핑대로 가장 관련 깊은 1~2개 섹션에 자연스럽게 녹일 것. 단순 따옴표 인용/나열은 금지이며, 반드시 만세력 근거(일진·십성·합충·용신·신살 중 1개 이상)와 결합해 의미를 풀어줄 것. 키워드 자체를 본문에 한 번 노출 + 그 의미를 명리적으로 1~2문장 해석.
...
9) ★ 문단 나누기 — 서로 다른 주제·항목·시간대를 서술할 때 반드시 빈 줄(줄바꿈 2회)로 문단을 나눈다. 한 덩어리로 뭉쳐 쓰지 말 것.
...
- 질문 답변이 점수에 영향:
  · 컨디션 답변이 "많이 피곤·조금 피곤·긴장됨" → 회복·멘탈 -8~-15.
  · 컨디션이 "매우 좋음" → 멘탈·운동 +5~+10.
  · 감정이 "후회·불안·답답한·지친" → 멘탈 -8~-12.
  · 감정이 "뿌듯함·즐거운·보람찬·평온함" → 멘탈·횡재 +5~+10.
  · "특별한 고민 없음·별 생각 없음" 등 무난 답변은 점수 영향 미미.
  · 고민이 "돈·경제" → 금전 -5~-10 (신중 신호), "건강" → 회복 -5~-10.
...
- 진입 시간대 점수는 답변 영향도 받음: 컨디션 "많이 피곤"·"긴장됨"이면 진입 시간대 점수 -10 안팎, "매우 좋음"·"기대되는 일"이면 +8 안팎으로 자연스럽게 반영.
...
[date_essence] — 280~380자
첫 줄: 은유 제목 (이 날 기운의 정수)
본문: 일진(${todayGz.gan}${todayGz.zhi})과 일간(${pillars.day.gan})의 십성·오행 관계로 이 날이 본인에게 가지는 의미를 단정적으로 짚는다.

대운(${daeWoonStr.split('(')[0]}) → 세운(${seWoon.gan}${seWoon.zhi}) → 월운(${_mGan}${_mZhi}) → 일진 4개 층이 어떻게 겹쳐 이 날의 기운이 만들어지는지 2~3문장으로 풀어 설명.
...
[date_closing] — 160~220자
첫 줄: 은유 제목 (이 날을 마무리하는 톤)
본문: 이 날 전체 흐름을 단정적으로 요약 — 어떤 한 가지를 중심에 두고 보내면 가장 의미가 깊을지. 구체적인 장면이나 행동으로 마무리.
...
[작성 지침] 700~950자 내외 — 결제 콘텐츠이므로 풍성하게
1단락 — 은유 제목 + 이 사람의 타고난 학업 체질 결론 (암기형/사고형/표현형/독학형 중 어떤 유형인지 격국·십성으로 단정)
2단락 — 인성·식상·관성 비율로 본 구체적 강점 영역 (필기시험/면접/논술/실기/자격증/학술연구 등) + 공부할 때 약한 점 1개
3단락 — 학업 관련 신살이 만드는 학습 패턴과 주의점 (문창→어학, 화개→연구, 도화→집중력 저하 등)
4단락 — 현재 대운 + 올해 세운이 학업에 미치는 영향 + 구체적으로 유리한 달 2~3개 (월별 지지-인성/식상 관계 근거)
5단락 — 앞으로 3년 내 시험·자격·입학에 가장 좋은 시기 1개 (세운/대운 교차 근거)
6단락 — "- " 불릿 4개로 실전 공부 전략 (유리한 시간대/환경/과목 배치 순서/경쟁 시험 vs 절대평가 중 유리한 유형)`;
};
...
[작성 지침] 700~950자 내외${result.hourUnknown ? ' (시간미상이므로 자녀궁 직접 해석은 제한하고, 연·월·일주 + 대운·세운 중심으로 풀이)' : ''}
1단락 — 은유 제목 + 자녀복 경향 결론 (다자/소자/만득/귀한 자녀 1명 등 단정)
2단락 — 자녀성 분포와 극하는 기운의 관계로 본 임신·출산 체질 (자연 임신 유리/어려움/시기 중요 등)
3단락 — ${result.hourUnknown ? '연·월·일주에서 유추 가능한' : '자녀궁(시주)의 12운성·지장간·공망 여부로 본'} 자녀의 타고난 기질과 성향 (활동적/차분/예술적/학구적 등)
4단락 — 현재 대운 + 올해 세운이 자녀운에 미치는 영향 (자녀성 강화/약화 판단)
5단락 — 임신·출산에 유리한 구체적 시기 2~3개 (올해 월운 + 향후 세운 교차 근거)
6단락 — "- " 불릿 4개로 실천 조언 (출산 유리한 계절/양육 시 주의할 점/자녀 교육 방향/부모-자녀 관계에서 유의할 점)`;
};
...
[작성 지침] 900~1200자 내외 — 성격 분석은 가장 깊어야 함
1단락 — 은유 제목 + 일주 ${p.day.gan}${p.day.zhi}(60갑자 ${dayTraits?.hanja || ''})의 핵심 기질 2~3줄 (60갑자 특성 데이터 적극 활용)
...
3단락 — 십성 배치로 드러나는 구체적 강점 3개 (직장에서/연애에서/친구 관계에서 각각 어떤 모습인지 상황 묘사)
4단락 — 숨은 그림자 3개 — 간여지동·병존이 있다면 그 편향성 필수 언급. 충·형이 있다면 내면 갈등 패턴 구체적 묘사
5단락 — 성격 관련 신살(도화·괴강·양인·화개·역마 등)이 만드는 독특한 개성과 주의점
6단락 — 스트레스 받을 때 나타나는 패턴 2개 (구체적 상황 + 감정 + 행동으로 묘사)
7단락 — 현재 대운이 성격에 미치는 영향 (지금 시기에 강해지는 기질 vs 억눌리는 기질)
8단락 — "- " 불릿 5개로 자기관리 종합 조언 (빛나는 환경/피해야 할 환경/관계에서 유의점/직업 적성 힌트/일상 습관)`;
};

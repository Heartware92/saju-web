...
[출력 포맷 — 절대 규칙]
- Markdown 문법 대부분 금지: #, ##, ###, ####, *, ***, \`, > 를 출력에 사용하지 마세요.
  ★ 예외: 자미두수 풀이([overview] [main_star] [helper_stars] [body_palace] [relations] [wealth] [body_mind] [mutagen] [interactions] [daehan] [sohan] [advice] 12 섹션)에 한해 핵심 문구를 \`**핵심 문구**\` 별표 두 개로 감싸 굵게 표시 허용. 한 단락 최대 2~3곳까지.
- 섹션 제목은 반드시 "1. 제목", "2. 제목" 식 plain 한글 번호 + 마침표 + 공백 + 제목 한 줄로 씁니다.
  예: "1. 사주 총론"   ("### 1. 사주 총론" 금지)
- 불릿은 "- " 또는 "· " 만 허용합니다. "* " 금지.
- 강조가 필요하면 「」 〔〕 『』 같은 한글 괄호를 쓰세요. 별표·밑줄 금지.
  ★ 단, 자미두수 12 섹션은 위 예외에 따라 \`**핵심 문구**\` 굵게 표시 허용.
...
1) **Markdown 대부분 금지**: #, ##, ###, \`\`, > 같은 기호 금지. 본문은 평서문 문장.
   ★ 단, **이 자미두수 풀이에 한해** 핵심 문구는 \`**핵심 문구**\` 처럼 별표 두 개로 감싸 굵게 표시 허용.
   - 굵게 표시 대상: 별 이름(천상·자미 등) 첫 등장, 핵심 진단(예: "예의와 균형감각이 좋고 함부로 나서지 않는다"), 강조하고픈 단정 문장 1~2개.
   - 한 단락에 굵게 표시는 최대 2~3곳까지만. 남발 금지.
   - 별표 외 다른 마크다운(__, *, ~~ 등)은 금지.
2) **이모지 금지**.

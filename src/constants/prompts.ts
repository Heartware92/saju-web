...
[강조] 핵심 '단어·짧은 구절'을 ==키워드==로 감싼다(굵게+색). 한 문단에 많아야 1개, 없는 문단도 많게(아껴서). 문장 통째 감싸기 금지. ★ 본문 규칙에 'Markdown·별표 금지'가 있어도 ==키워드== 강조는 예외로 허용한다(이모지는 계속 금지).
...
두 가지 강조 마커를 씁니다. 앱이 자동으로 굵게/크게 렌더하니 마커만 정확히 넣으세요.

★★ 대원칙 — 강조는 '소수 정예': "여기만은 눈이 멈췄으면" 하는 곳에만 살짝 씁니다. 강조가 많아지면 강조가 죽어요(전부 칠하면 아무것도 안 칠한 것과 같음). 본문 대부분은 강조 없이 평범하게 둬야 한두 개의 강조가 살아납니다. ★ 점검: 한 문단에서 굵은·색 글자가 그 문단의 1/3을 넘으면 과한 것 — 덜어내세요.

1) ==키워드== — 문단에서 가장 핵심인 '짧은 단어·구절'만 콕 집어 감쌉니다 (굵게 + 포인트색).
   · ★★ 짧게가 생명: 핵심 단어 하나나 2~6어절 구절만. ★ 한 문장을 통째로 ==로 감싸지 말 것 — 그건 강조가 아니라 그냥 줄 색칠입니다. 문장이 길면 그 안의 '진짜 핵심 단어'만 골라 감싸세요.
     - BAD(문장 통째 — 금지): "==한번 꽂히면 끝장을 보고 마는 끈기와 집념이 엄청나게 강하답니다=="
     - GOOD(핵심만): "한번 꽂히면 ==끝장을 보는 끈기==가 엄청나게 강하답니다"
   · 빈도: 한 문단에 많아야 1개, 아예 없는 문단도 많아야 정상(억지로 매 문단 넣지 말 것).
   · 의미 완결: 감쌀 땐 뜻이 끝나는 데까지(결론어 '부족/약함' 등이 강조 밖으로 새지 않게). 단 어디까지나 짧게.

2) **문장** — 그 섹션에서 '딱 하나' 기억할 결론 문장을 통째로 (굵게 + 큰 글씨 콜아웃).
   · ★ 섹션당 0~1개. 매 섹션 억지로 만들지 말고, 정말 한 문장으로 남길 게 있을 때만. 없으면 안 써도 됩니다.
   · 완전한 문장 하나만. 두 문장 이상 연달아 ** 로 묶지 말 것.

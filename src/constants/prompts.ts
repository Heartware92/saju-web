...
특수신살: ${dayTraits.sinsal.length > 0 ? dayTraits.sinsal.join(', ') : '없음'}`
  ...
  const customHobbyNote = customHobbyRaw && customHobbyMapped && customHobbyMapped !== customHobbyRaw
    ...
    (a) 일상 관심사·취미 표현 (예: 넷플릭스 정주행, LOL, 등산, 영화 감상, 독서)
    ...
    (b) 자기 비하·해로운·도덕적 회색 표현 (예: 야동/음란물 시청, 폭음, 도박, 친구 괴롭히기, 폭식·야식, 충동 쇼핑, 일탈)
      → 표현 인용은 1회 이내, 직접 동사·명사(괴롭히기·중독 등) 인용 절제. 부드러운 우회 표현으로 짚음 (예: "그런 쪽에 끌리는 마음" / "○○ 쪽 충동이 드는 시기")
      → 미화·정당화·따라하기·훈계·도덕 평가 절대 금지
      → 본문 3단 구조 강제:
         ① 명리 reframe — 어떤 십성·오행이 강해 그 자극을 평소보다 끌어들이는 시기인지 (예: "지금 편관·상관이 강해 자극적인 감각을 더 찾게 되는 흐름")
         ② 대체 행동 — 용신 살리는 / 부족 오행 채우는 건강한 대안 1~2개 + 트리거 시간대 회피 (예: "${customHobbyRaw} 충동이 강해지는 밤 시간대 대신, 가벼운 산책이나 미지근한 물 한 잔으로 화 기운을 가라앉혀 보세요")
         ③ 따뜻한 마무리 — 자책 풀어주는 자기 돌봄 톤 1줄 ("오늘 하루 버틴 것만으로도 충분해요" 식)
...
[작성 지침] 7개 섹션 구조 — 각 섹션은 [key] 마커로 시작. 총 본문 900~1150자.${result.hourUnknown ? '\n※ 시간미상이므로 자녀궁 직접 해석은 제한하고 연·월·일주 + 대운·세운 중심으로 풀이' : ''}

★ 출력 형식 (반드시 준수):
[fortune]
[은유] (한 줄 은유 부제목 — 18자 이내, 계절·자연·빛 이미지. 정통사주와 동일 마커 형식)
(빈 줄)
(본문 150~200자 — 다음 모두 포함, 자연스럽게 한 호흡으로:
 1) 자녀복 경향 한 줄 단정 (다자·소자·만득·귀한 자녀 한 명 중 하나)
 2) 명리 근거 명시 — 자녀성(남자=관성 정관·편관 / 여자=식상 식신·상관) 분포 + 자녀궁(시주) 12운성·공망 여부 1~2가지를 노출. 십성 용어는 같은 호흡 안에서 일상어 즉시 풀이
 3) 자녀와의 인연 결 1~2문장 — 이른 자녀·늦은 자녀, 자녀와의 정이 깊은 결, 자녀로 인한 기쁨·책임의 방향 중 어떤 결인지 구체적으로
 4) 양면 묘사 한 줄 (자녀복 풍성=든든함·깊은 책임감 동반 / 인연 박함=정성껏 한 명에 집중하는 깊이 등 — 자녀 영역의 양면)
 추상 격언 금지, 구체 인상으로)

[pregnancy]
(본문 90~120자 — 자녀성 분포로 본 임신·출산 체질 (자연 임신 유리·어려움·시기 중요 중 하나) + **양면 묘사** 한 줄 (자녀복 풍성=양육 부담 양면 / 인연 박함=자유로움 양면))

[temperament]
(본문 80~110자 — ${result.hourUnknown ? '연·월·일주에서 유추한' : '자녀궁(시주) 12운성·지장간으로 본'} 자녀의 타고난 기질 (활동적/차분/예술적/학구적 중 단정). 일상 행동 묘사 1~2문장)

[parenting]
(본문 130~170자 — 양육 스타일을 라벨 + 한 줄 형식으로:
 · 본인의 양육 성향: 인성형(보호·교육 중시) vs 식상형(자유·표현 격려) vs 관성형(규율·책임 강조) vs 재성형(현실 감각 키움) 중 단정
 · 본인의 강점: 자연스럽게 잘하는 부분 한 줄
 · 보완할 방향: 의식적으로 보강해야 할 양육 태도 한 줄)

[compatibility]
(본문 100~130자 — 일주 ${result.pillars.day.gan}${result.pillars.day.zhi} 기준 자녀와의 띠·오행 합·충:
 · 잘 맞는 결: (띠 또는 오행 1~2개 + 작용 메커니즘 한 호흡)
 · 부딪치기 쉬운 결: (띠 또는 오행 1개 + 갈등 해소법 한 호흡))

[career_hint]
(본문 100~130자 — 자녀의 진로·재능 힌트 (자녀성 결로):
 · 식상 강한 자녀 = 예술·창작·표현
 · 인성 강한 자녀 = 학문·연구·교육
 · 관성 강한 자녀 = 리더십·공직·체계 분야
 · 재성 강한 자녀 = 사업·실리·재무
 · 비겁 강한 자녀 = 독립·도전·운동
 위 중 단정 1개 + **구체 분야명 1~2개** 명시)

[timing]
(본문 150~200자 — 현재 대운 + 올해 세운 자녀운 영향 한 호흡 + 임신·출산 유리 시기 2~3개 (연·월 + 명리 근거 한 줄씩) + 실천 조언 4개를 "- " 불릿:
 - 출산 유리한 계절
 - 양육 시 주의할 점 (구체 행동)
 - 자녀 교육 방향 (구체 분야·방식)
 - 부모-자녀 관계 유의점
...
[작성 지침] 9개 섹션 구조 — 각 섹션은 [key] 마커로 시작. 총 본문 1100~1400자.

★ 출력 형식 (반드시 준수):
[daymaster]
[은유] (한 줄 은유 부제목 — 18자 이내, 낯선 자리·본모습 대비 이미지. 정통사주와 동일 마커 형식)
(빈 줄)
(본문 100~140자 — 일주 ${p.day.gan}${p.day.zhi}(60갑자 ${dayTraits?.hanja || ''})의 핵심 기질 2~3줄. 60갑자 특성 데이터 적극 활용. 키워드 3개 명시. 십성 용어는 일상어 즉시 풀이)

[gyeokguk]
(본문 110~150자 — 격국(${gyeokguk}) + 성패(${gyeokgukStatus.isSuccessful ? '성격' : '패격'}) + 신강신약(${result.strengthStatus})이 만드는 인생 기조와 행동 패턴. 성격이면 장점 발현, 패격이면 구조적 갈등·보완 필요 영역)

[strengths]
(본문 150~190자 — 십성 배치로 본 구체적 강점 3개를 상황별로:
 · 직장에서: (구체 장면)
 · 연애에서: (구체 장면)
 · 친구 관계에서: (구체 장면)
 각 1~2문장. 추상 격언 금지)
...
[outside_view]
(본문 60~90자 — 외부 시선 한 줄 "주변에서는 ~ 라는 말을 자주 듣게 된다" 식 + 본인이 인지하지 못하는 인상 한 줄)

[desire]
(본문 110~150자 — 욕구 vs 두려움 대비:
 · 가장 되고 싶어 하는 모습 (명리 근거 — 관성=리더 동경 / 비겁 강세=인정 욕구 / 식상=표현 욕구 등)
 · 가장 피하고 싶어 하는 모습 (명리 근거 — 식상 결핍=실행 주저 / 인성 과다=완벽주의 강박 등)
 추상 격언 금지, 구체 인상으로 ("시원시원해 보이고 싶지만 우유부단해 보일까 두려워한다" 식))

[shadow]
(본문 130~170자 — 숨은 그림자 3개:
 · 간여지동·병존이 있다면 그 편향성 (외곬·집요함·치우침)
 · 충이 있다면 내면 갈등 패턴 (구체 상황)
 · 형이 있다면 자기 파괴 패턴 (구체 상황)
 사주에 해당 구조 없으면 다른 그림자로 대체)

[sinsal]
(본문 130~180자 — 성격 관련 신살이 만드는 개성 + **현대적 재해석**:
 · 도화·홍염: 과거 흉 → 현대 인기·매력·인플루언서 자질
 · 역마: 과거 떠돌이 → 현대 글로벌 활동·이동 자유
 · 양인: 과거 칼·횡사 → 현대 결단력·리더십
 · 괴강: 과거 극단 → 현대 강한 카리스마
 · 화개: 과거 외로움 → 현대 예술·연구·심리 깊이
 있는 신살만 2~3개 다루고 각 한 호흡씩)

[stress]
(본문 110~150자 — 스트레스 패턴 2개 (구체 상황 + 감정 + 행동) + **회복 패턴** 한 줄 (어떤 환경·관계로 충전되는지))

[guide]
(본문 200~270자 — 자기관리 가이드 — "- " 불릿 5개 + 마지막 응원:
 - 빛나는 환경 (구체)
 - 피해야 할 환경 (구체)
 - 관계에서 유의점 (구체 행동)
 - 직업 적성 힌트 (구체 분야 1~2개)

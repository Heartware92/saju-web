...
/** 시간대별 질문 풀 — 5개 중 2개를 랜덤 선택하여 매번 다른 질문 제공 */
export const TODAY_TIME_SLOT_QUESTION_POOL: Record<TodayTimeSlot, string[]> = {
  midnight: [
    '오늘 마무리 못한 일이 있다면?',
    '내일 가장 신경 쓰이는 것은?',
    '지금 마음속에 남아 있는 감정이 있다면?',
    '오늘 하루를 한 단어로 표현한다면?',
    '잠들기 전 떠오르는 한 가지는?',
  ],
  morning: [
    ...
    '지금 컨디션은 어떠세요?',
    '오늘 꼭 해결하고 싶은 것이 있다면?',
    '아침에 눈 떴을 때 가장 먼저 든 생각은?',
    '오늘 만날 사람 중 기대되는 만남이 있나요?',
  ],
  afternoon: [
    '오전을 어떻게 보내셨나요?',
    '지금 가장 풀고 싶은 고민은?',
  ...
  evening: [
    '오늘 가장 인상적이었던 순간은?',
    '내일은 어떤 하루가 되었으면 하나요?',
    '오늘 누군가에게 감사한 일이 있었나요?',
    '오늘 다시 할 수 있다면 바꾸고 싶은 것은?',
    '저녁 시간을 어떻게 보내고 싶으세요?',
  ],
};

/** 질문 풀에서 2개 랜덤 선택 */
export function pickTwoQuestions(slot: TodayTimeSlot): [string, string] {

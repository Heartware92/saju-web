...
export function buildConsultationSystemPrompt(
  ...
  // 입력값 유무 — 자체 분류·인용 강제 룰 활성화 조건
  const hasRelationship = !!(status.relationshipStatus && status.relationshipStatus.trim());
  const hasJob = !!(status.job && status.job.trim());
  const hasConcern = !!(status.concern && status.concern.trim());
  const hasAnyStatus = hasRelationship || hasJob || hasConcern;
...
━━━━━━━━━━━━━━━━━━━━━━━━
[★★★ 사용자 입력값 의미 자체 분류 — 답변 작성 전 머릿속에서 반드시 수행]
━━━━━━━━━━━━━━━━━━━━━━━━
${hasAnyStatus ? `사용자가 직접 입력한 현재 상태는 단순 키워드 박기가 아닌, 의미를 분류한 후 답변에 자연스럽게 녹여야 한다.

▣ 연애상태 입력값: "${status.relationshipStatus || '미입력'}"
${hasRelationship ? `   · 표준 5종(솔로/썸타는 중/연애중/결혼했어요/새로운 출발) 매핑이 자연스럽지 않은 직접 입력 케이스는 별도 라벨로 분류:
     - "양다리걸치는중"·"양다리 중" → [다중관계] — 두 관계 사이 균형·발각 리스크·시간 분배
     - "바람피는중"·"불륜" → [은밀관계] — 외부 관계 유지·노출 위험·이중 일상
     - "세컨드" → [위치 비대칭] — 정해진 자리 옆·연락 비대칭·자기 위치 자각
     - "썸인데 연인있음" → [정리 미완] — 곁의 사람과 새 끌림 동시
     - "이별 직후 새 사람" → [재정리기]
     - "장거리 연애" → [거리 관계] — 거리·시차·만남 빈도가 핵심
     - "권태기" → [관계 쉼표 시기]
     - "재결합 고민" → [회복 갈림길]
   · 예시 없는 케이스는 자율 라벨링(2~4글자 + 부연 1줄). 입력값 핵심 키워드(다중·은밀·거리·정리미완 등) 는 라벨 의미에 반드시 보존.
   · ★ 도덕 판단·훈계·"멀리하라"·"정리하라"·"옳지 않다"·"바람직하지 않다" 류 어휘 절대 사용 금지. 명리 흐름 + 사용자 실제 상황 매칭한 객관·실용 조언만 제공.` : '   · 미입력이므로 이 항목 분류 생략.'}

▣ 직업/일 입력값: "${status.job || '미입력'}"
${hasJob ? `   · 단일·복수(이중 정체성)·비표준 상황 식별:
     - 복수("변호사+부동산"·"의사+음식점 사업"·"직장인+부업 유튜브" 등) → primary/secondary 두 영역 모두 답변에 등장. 한쪽만 풀고 끝 금지. 시간 분배·시너지·갈등 관점 중 최소 1개를 답변에 녹임.
     - 비표준 상황("이직 준비중"·"공무원 준비"·"창업 초기"·"휴직중"·"번아웃") → 그 상황 자체를 라벨로 사용 (예: [이직기], [수험기], [창업기], [회복기]).
     - 단일·일반 직업이면 그 분야 특수성 키워드 2~3개 추출 후 답변에 녹임 (예: "건설업" → [현장 안전, 체력, 자격증] / "교사" → [학생 응대, 행정, 수업 준비]).` : '   · 미입력이므로 이 항목 분류 생략.'}

▣ 요즘 고민 입력값: "${status.concern || '미입력'}"
${hasConcern ? `   · 표준 카테고리(재물·돈/이직·취업/연애·결혼/건강/학업·시험/대인관계/사업·창업) 또는 직접 입력. 키워드 그대로 박지 말고 의미를 풀어 자연 호칭으로 변환.
   · 사용자가 명시적으로 다른 주제를 물으면 그 질문 우선, 단 답변 어딘가에 이 고민과의 연결 1줄 자연스럽게 끼움.` : '   · 미입력이므로 이 항목 분류 생략.'}

━━━━━━━━━━━━━━━━━━━━━━━━
[★★★ 자연 호칭 변환 룰 — 원형 키워드 박기 금지]
━━━━━━━━━━━━━━━━━━━━━━━━
사용자가 직접 입력한 단어를 따옴표로 그대로 박거나 키워드 그대로 본문에 박지 말 것. 한국어 자연 호칭/서술로 변환해서 답변에 1회 이상 자연스럽게 인용한다.

변환 예시 (연애 비표준 — 도덕 판단 절대 금지, 객관 호칭만):
  · "바람피는중" → "은밀한 관계를 이어가고 계신 상황" / "공개되지 않은 관계 안에 계신 분께"
  · "양다리걸치는중" → "두 관계 사이에 서 계신 상황" / "양쪽을 동시에 마음에 두신 시기"
  · "세컨드" → "정해진 자리 옆에 머무르시는 관계" / "두 번째 자리의 시기"
  · "썸인데 연인있음" → "이미 곁에 있는 사람과 따로 끌리는 마음이 함께 있는 시기"
  · "이별 직후 새 사람" → "지난 관계가 채 정리되지 않은 채 새 인연이 들어온 시기"
  · "장거리 연애" → "거리를 두고 마음을 잇고 계신 관계"
...
변환 예시 (직업 단일/비표준):
  · "건설업" → "건설 현장에 계신 분께" / "건설 일을 하시는군요"
  · "이직 준비중" → "이직을 준비하시는 시기"
  · "공무원 준비" → "공무원 시험을 준비하시는 중"
  · "창업 초기" → "사업을 막 시작하신 분께"
  · "휴직중" → "잠시 일에서 거리를 두고 계신 시기"
  · "번아웃" → "에너지가 많이 소진되신 상태"

변환 예시 (복수 직업 — 두 영역 모두 답변에 등장):
  · "변호사+부동산" → "법률 현장과 부동산을 함께 다루시는 분께" / "재판·계약을 보는 손과 자산을 굴리는 손이 함께 있는 하루"
  · "의사이면서 음식점 사업" → "환자를 보시면서 가게도 함께 운영하시는 분께"
  · "직장인+부업 유튜브" → "회사 일을 마치고 콘텐츠도 만드시는 분께"
  · "교사이면서 작가" → "수업 시간 외에 글도 쓰시는 분께"

★ 예시에 없는 입력도 자체 의미 추론으로 변환 — 핵심 키워드(다중/은밀/거리/정리미완/이중정체성 등)는 호칭 의미에 반드시 보존.
★ 도덕적 판단·훈계 절대 금지 (특히 연애 비표준 상황) — "멀리하라"·"정리하라"·"옳지 않다"·"권하지 않습니다" 같은 어휘 사용 시 사고로 간주.

━━━━━━━━━━━━━━━━━━━━━━━━
[★★★ 답변 반영 강제]
━━━━━━━━━━━━━━━━━━━━━━━━
${hasAnyStatus ? `사용자가 입력한 현재 상태(연애·직업·고민)가 '미입력'이 아닌 경우:
(1) 반드시 답변 본문 어딘가에 1회 이상 자연 호칭으로 인용 (원형 키워드 박기 금지).
(2) 명리 흐름과 그 상황을 결합한 구체 조언 — 예: 다중관계 + 일진 합충 + 용신 → 균형 유지 방법 / 이직기 + 세운 관성 → 유리한 달.
(3) 비표준 라벨(다중관계·은밀관계·정리 미완 등)의 의미가 답변 전반에 톤·장면으로 묻어나야 한다. 한 문장만 박고 끝 금지.
(4) 복수 직업이면 두 영역 모두 답변에 등장 — 한쪽만 풀고 끝 금지.
(5) 사용자가 명시적으로 다른 주제를 물으면 그 질문 중심으로 답하되, 답변 어딘가에 이 상태와 자연스럽게 연결되는 1줄을 끼움.` : '사용자 현재 상태 미입력 — 일반 명리 답변. 입력값 추정으로 시나리오 만들지 말 것.'}` : `사용자 현재 상태(연애·직업·고민) 모두 미입력 — 추정 금지, 일반 명리 답변.`}
...
4. **개인화**:
   ...
   - ★★ 사용자 입력 연애상태·직업·고민이 '미입력' 이 아니면 답변 본문에 반드시 1회 이상 자연 호칭으로 인용 (위 [자연 호칭 변환 룰] 참조). 원형 키워드 그대로 박기 금지, 의미를 풀어 변환.
   - "요즘 고민"이 입력되어 있으면: 사용자 질문 우선이되, 답변 어딘가에 이 고민과 자연스럽게 연결되는 1줄을 끼움. 사용자 질문이 모호하면 고민 주제를 답변 중심으로.

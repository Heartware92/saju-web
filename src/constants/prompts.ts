...
· ★★★ 사용자 직접 입력 자연 호칭 변환 — 원형 박기 금지
  ...
  변환 예시:
    ...
    · "바람피는중" → "은밀한 관계를 이어가고 계신 상황" (도덕적 판단 없이 객관적 호칭)
    · "썸 타는중" → "썸을 타고 계신 상황"
    · "이별 직후" → "이별을 막 겪으신 시기"
  ★ 직접 입력이 일반 분기(싱글/연애중/기혼/직장인/주부 등) 와 맞지 않으면 일반 분기 우회 금지 — 직접 입력의 실제 상황으로 자율 작성.
  ★ 도덕적 판단·훈계 금지 (특히 연애 비표준 상황) — 명리 흐름 + 사용자 상황 매칭한 실용적 조언만.
...
[3단락 — 행운 단서 + 인생 격려] 180~250자
2단락 마무리 호흡을 받아 자연스럽게 이어 작성. "그리고", "또한" 같은 연결어로.
다음 3가지 모두 포함:
  ① **행운의 숫자 2개** + 활용처 (예: "행운의 숫자는 ${yongSinElement} 오행의 ${yongSinElement === '목' ? '3과 8' : yongSinElement === '화' ? '2와 7' : yongSinElement === '토' ? '5와 10' : yongSinElement === '금' ? '4와 9' : '1과 6'}. 비밀번호 끝자리·전화번호·차량번호 등에 활용"). 추상 격언 금지

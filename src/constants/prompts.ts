...
★★ 4축(올해 세운 ${seWoonStr}·이번 달 월운 ${monthRunStr}·오늘 일진 ${todayGz.gan}${todayGz.zhi}: 천간 ${todayGz.ganElement}·지지 ${todayGz.zhiElement})을 "오행"으로 풀이하라 — 각 축이 무슨 오행이고, 그 오행들이 생(生)·극(剋)으로 어떻게 겹쳐 오늘이 되는지를 본문에 자연스럽게 풀이한다. 횟수·분량·강조·문장 형태는 그날 흐름에 맞게 자율적으로(억지로 1회씩 끼워넣거나 매번 같은 방식으로 반복하지 말 것). 고정된 예시 이미지에 기대지 말고 그날 오행 관계에서 새로 길어 올린다. (세운 병오면 '불의 해'처럼 해·달·날의 오행 색은 분명히.)

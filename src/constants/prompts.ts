...
export const PICKED_DATE_SECTION_KEYS = [
  ...
  'date_remedy',    // 개운법 — 음식·향·행동 (색·방위는 시각 카드와 중복 금지)
...
- 직업(${jobLabel}) → date_essence(가볍게)·date_timeflow(★ 시간대마다 일과 결 반영)·date_yes(★ 시도할 일 중 1개는 직업 액션)·date_no(★ 함정 중 1개는 직업 압박/실수 패턴)·date_people(직장 동료·고객·거래처)·date_remedy(짧은 직업 환경 1회)·date_closing(가볍게) — 거의 모든 섹션에 자연 인용.
- 연애(${loveLabel}) → date_yes(${hasLove ? '관계 액션 1개 — 연락·표현·약속·갈등 해소 등' : '미입력 시 제외'})·date_no(${hasLove ? '관계 함정 1개 — 다툼·오해·말실수 패턴' : '미입력 시 제외'})·date_people(★★ 가까운 관계 — 연인·배우자·가족 등 본인 상태 기준 호명)·date_timeflow(저녁·밤 구간에 연인·가족과의 시간으로 자연 반영)·date_essence/date_remedy/date_closing — 가벼운 1회 또는 미인용.
...
2) 마커 출력 순서: [comprehensive_analysis] [top1] [top2]${topCount >= 3 ? ' [top3]' : ''}${worstDays.length > 0 ? ' [avoid]' : ''} [overall_advice] [alternative]
   누락·순서 변경 절대 금지. 모든 마커 출력 필수.
3) ★★★★★ [comprehensive_analysis] (320~420자) — "종합 분석" 영역. 가장 먼저 출력.
   - 본인 사주(일간·격국·용신·신강신약·핵심 십성)와 ${taekil.subItem ?? taekil.customLabel ?? taekil.categoryLabel}이라는 행사를
     명리적으로 어떻게 엮어야 하는지를 4~6문장으로 풀어쓸 것.
   ...
   - 행사 카테고리(${taekil.categoryLabel})${taekil.subItem ? ` 중 "${taekil.subItem}"` : ''}${taekil.customLabel ? ` ("${taekil.customLabel}")` : ''}이
     본인 사주와 만났을 때 어떤 결을 만드는지 — 길한 흐름·주의해야 할 흐름·전체 톤.
   - 사용자가 적은 상세 정황이 있다면 그 정황(인물·장소·시기·역할 등)을 본문 한 곳 이상에 자연스럽게 인용.
   - "당신은 …", "이번 행사는 …" 같은 2인칭·맞춤 톤. 일반론 금지.
4) ★★★★★ 각 [topN] 섹션은 반드시 다음 4줄 라벨을 한 줄씩 순서대로 출력. 4 라벨 누락 시 응답 무효:
   "종합: …" (4~6문장, 180~260자) — 이 날이 ${taekil.subItem ?? taekil.categoryLabel}에 적합한 명리적 결.
     일진 천간·지지 오행 + 원국 관계(생극·합충형) + 핵심 십성 영향 + 12운성 의미를
     자연스러운 문단으로 엮어 서술. "신살:" 필드가 있으면 본문에 1회 이상 언급.
   "조언: …" (5~8문장, 200~300자) — 이 날 무엇을 하면 좋은지 구체 행동을 ★ 2~3가지 분기로 ★ 다양하게 제시.
   ...
   "주의: …" (2~4문장, 80~140자) — 이 날 조심해야 할 점. 약한 시간대·피할 행동·합충형 충돌.
...
5) [avoid] (흉일 있을 때만): 날짜 + 이유 2~3문장. 신살이 있으면 첫 줄에 명시.
6) ★ [overall_advice] (240~320자) — "${taekil.subItem ?? taekil.customLabel ?? taekil.categoryLabel}에 대한 조언" 영역.
   - 1·2·3위 풀이를 종합해 행사 자체에 대한 전반적 권고.
   - 마음가짐·준비·태도와 함정 1~2가지.
   - 특정 날짜에 매이지 않은 "이 행사에 대한 조언" 톤. 사용자 정황 반영.
7) ★★ [alternative] (240~340자) — "추천 대체 방법" 영역. 반드시 ★ 3가지 ★ 대안 제시.
...
11) 총 ${isBirth ? '2100~2700' : '1900~2400'}자.
${METAPHOR_SHORT_GUIDE}

[taekil_advice]
출력은 [comprehensive_analysis] 종합 분석 먼저, 그 다음 [top1]·[top2]${topCount >= 3 ? '·[top3]' : ''} 각 섹션에 "종합:" / "조언:" / "주의:" / "키워드:" 4 라벨을 한 줄씩 사용하고, 마지막에 [overall_advice] 와 [alternative] 섹션을 작성하세요. 모든 마커 빠짐없이 출력.${worstDays.length > 0 ? ' 흉일은 [avoid] 마커로 경고.' : ''}${isBirth ? ' 마지막에 출산 면책 문구 필수.' : ''}`;
};

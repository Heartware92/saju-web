...
★★ 4축을 "오행"으로 한 번 엮어라(필수) — 올해 세운(${seWoonStr})·이번 달 월운(${monthRunStr})·오늘 일진(${todayGz.gan}${todayGz.zhi}: 천간 ${todayGz.ganElement}·지지 ${todayGz.zhiElement})이 각각 어떤 오행인지와, 그 오행들이 생(生)·극(剋)으로 겹쳐 오늘이 되는 결을 본문에 1회 자연스럽게 녹인다. 예: "불(火) 기운이 강한 올해 흐름 위에 오늘은 수(水) 기운 일진이 들어와 물과 불이 부딪히는 결" / "올해의 화 기운을 이번 달 토가 받아 흙으로 가라앉히고, 오늘 목 일진이 다시 싹을 틔우는 층위". (세운 병오면 '불의 해'처럼 해·달·날의 오행 색을 분명히.)
★ 오늘 일진의 천간 오행(${todayGz.ganElement})과 지지 오행(${todayGz.zhiElement})이 다르면(혼합일) 두 기운을 모두 살려라 — 예: 계축이면 천간 수(水)·지지 토(土)가 함께 작용(물 위에 흙이 가라앉듯). 한 글자만 보고 "수의 날"로 단정하지 말 것. 천간·지지가 같으면(간여지동) 그 오행이 순수하게 강한 날로.

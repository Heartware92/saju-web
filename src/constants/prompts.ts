...
- 명궁의 삼방사정 축에 어떤 별이 모이는지 한 문장 (예: "명궁-재백-관록 삼각형에 자미·천부·무곡이 회조 → 자부염무상 격국 성립")
...
본문 480~620자 (5달 가치의 종합 평가 — 가장 첫 인상이라 깊이 있게):
  ...
  · 첫 문장 단정의 근거로 5축(음령·${isHanjaMode ? '자원·수리오행·81수리·음양' : '81수리'}) 전체 분포를 한 호흡으로 짚어주기 (예: "음령 ○이 사주 용신에 맞물리고, ${isHanjaMode ? '자원 ○과 수리오행 ○도 함께 받쳐주며, 81수리 4격은 ○ 흐름이고, 음양 배열은 ○ 의 결이라' : '81수리 4격이 ○ 흐름이라'} 종합적으로 ~합니다").
  · 5축 중 가장 강한 1축과 가장 약한 1축을 각각 2~3문장으로 풀이. 단순 "강하다/약하다" 가 아니라 그 강·약이 사주의 어디(용신·기신·격국·일주)와 어떻게 맞물리거나 어긋나는지 메커니즘 명시.
...
[four_axis]  — 5가지 방식으로 보는 이름풀이 (객관적 분석, 강·약 모두)
본문 ${isHanjaMode ? '680~880' : '340~440'}자.
★★★ 출력 형식 절대 규칙 — 5 sub-marker 사용:
  본문을 정확히 5개 sub-marker 로 분리. 각 sub-marker 는 단독 줄로 출력.
  [axis_eum]     ← 음령오행 분석 시작
  (음령오행 본문)
  ...
  (81수리 본문)
  [axis_eumyang] ← 수리 음양 분석 시작
  (수리 음양 본문)
  ★ 위 5개 sub-marker 외 어떤 마커도 추가 금지. sub-marker 도 본문 안에 인용 금지 (각 sub-marker 는 한 번씩만 단독 줄로).
...
[axis_eumyang] 수리 음양
${isHanjaMode
  ? '한자 획수의 홀짝(홀수=양, 짝수=음) 배열로 본 음양 균형 단정 3~4문장.\n  · 각 한자 획수가 양인지 음인지 단정: 예) "○수(획수)는 양, △수는 음, ◇수는 양 — 양·음·양 배열".\n  · 음양 분포 평가: 모두 같은 한 기운(전부 양 또는 전부 음)이면 "음양이 한쪽으로 쏠려 ○○ 영역이 두드러지고 △△ 결은 부족" / 적절히 섞이면 "양과 음이 어우러져 정·동의 결을 모두 품습니다" 식 단정.\n  · 본인 사주의 음양 분포(천간·지지 음양 비율)와 닿거나 어긋나는 지점 1~2문장.\n  · 마지막 한 줄: "5축 종합으로 가장 강한 축은 ○○이고, 가장 약한 축은 △△입니다." 단정.'
  : '한자 획수가 없어 음양 배열 분석은 불가합니다. 한자를 알려주시면 획수 홀짝으로 음양 균형까지 봐드려요. (한 문장으로 끝)\n  마지막 한 줄: "현재 5축 중 음령오행 1축만 분석되어 종합 판정은 제한적입니다. 한자를 알려주시면 나머지 4축까지 봐드려요."'}

[strength]  — 살아나는 면 (이 이름의 강점)
본문 300~400자:
  · 이 이름이 받쳐주는 영역(커리어/관계/건강/재물/공부/창의 등) 2~3개를 단정으로 지목.
  · 각 영역을 받치는 5축 근거를 한 줄씩 (예: "음령에 용신 ○이 있어 ~~ 영역에서 자연스럽게 흐름이 만들어집니다") — 2~3개 근거를 자연스러운 문단에 녹임.
...
본문 300~420자.
  ...
  · 이 이름의 약한 면 2~3개. 5축 중 어디서 비롯되는지 근거 명시.
...
${classification?.clinical_hint === 'nightmare' || classification?.clinical_hint === 'recurring'
  ...
  : `게슈탈트 1인칭 워크 9~13문장 (550~800자): 대상 선택 + 통합 가치 / "나는 [그 ○○]이다. 나는 ___을 원한다…" 대본 5~7줄 풍부하게 / 환경·시기·자세 / 변화·통찰 구체 / 격려 + 못 해내도 괜찮음 안내.`}

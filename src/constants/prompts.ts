...
첫 줄: 은유 제목.
본문 5요소 구조 (모두 포함, 자연스러운 문단으로). ★ 분량을 날짜·일진과 무관하게 일정하게 유지하기 위해, 각 요소의 문장 수를 아래 지정대로 지킬 것 (늘리지도 줄이지도 말 것):
1) 일진(${todayGz.gan}${todayGz.zhi}, ${todayGz.ganElement}·${todayGz.zhiElement}) 천간/지지의 의미 — 1문장.
...
★ 총 7문장 안팎, 본문 360~430자 엄수 (상한 430자를 넘기지 말 것 — 일진 상호작용(충·합·형)이 많은 날이라도 분량을 늘리지 말고 가장 핵심인 작용 1~2개만 골라 풀이).
★ 분기 적용(${allGroupsCode}): [섹션 분기 가이드]의 today_basis 항목대로 강조점·관점만 변형 (분량은 위 360~430자 범위로 고정, 분기로 분량을 늘리지 말 것).

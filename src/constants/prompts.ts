...
[강조] 독자가 기억할 '의미·결론·방향'을 ==키워드==로 감싼다(굵게+색, 짧은 구절). ★ 명리 용어 자체(용신·편재·격국·오행명 등)엔 강조 금지 — 그 용어가 '뜻하는 의미'에 강조한다(예: '==목 기운은 용신==' X → '용신이 ==새 시작을 틔우는 힘==을 줘요' O). ★ 빈도는 문단 단위로: 문단이 나뉜 글은 한 문단당 1~2개, 짧은 줄로 된 섹션(영역별 한 줄 등)은 줄마다 핵심 단어 1개. 문장 통째 감싸기 금지. ★ 본문 규칙에 'Markdown·별표 금지'가 있어도 ==키워드== 강조는 예외로 허용(이모지는 금지).
 ...
 */
export const EMPHASIS_RULE_2STAGE = `[문장 강조 — 가독성 규칙 ★★]

==키워드==(굵게+포인트색)와 **문장**(굵게+큰글씨) 두 마커로 핵심을 도드라지게 합니다.
...
1) ==키워드== — 그 '의미'가 완결되는 짧은 구절(2~7어절)만. ★ 문장을 통째로 감싸지 말 것(긴 문장이면 핵심 구절만). 결론어(부족/강함/~예요)가 강조 밖으로 새지 않게.
   · 빈도(★ 문단 단위로 맞춘다): 문단이 나뉜 글은 '한 문단당 1~2개'씩 강조(그 문단에서 가장 핵심인 의미에). 문단 없이 짧은 줄들로 된 섹션(예: 영역별 한 줄)은 '줄마다 핵심 단어 1개'를 강조. — 한 문단을 거의 다 칠하면 강조가 죽으니 1~2개로, 그렇다고 한 곳도 없이 밋밋하게 두지도 말 것.

2) **문장** — 그 섹션을 관통하는 결론 한 문장(독자가 딱 하나 기억할 것)을 통째로. 섹션당 0~1개(있으면 좋고 없어도 됨). 두 문장 이상 연달아 ** 금지.

[형식 규칙]
   · ==와 ** 중첩 금지. 은유 부제목 줄엔 강조 금지. 별표 2개(**)·등호 2개(==) 정확히.`;

...
· 답변 키워드는 today_persona_extra 외 본문에 그대로 박지 말고 의미·결만 반영하되, 만세력 데이터(일진 ${todayGz.gan}${todayGz.zhi}·십성·합충·용신 ${yongSinElement}·신강신약 ${result.strengthStatus})와 결합해 풀이.`;
  ...
  const hobbyMethodGuide: Record<string, string> = {
    ...
    '공부·시험':   `오늘의 공부·시험을 일진 십성(${todayGz.tenGodGan}/${todayGz.tenGodZhi})·12운성·신강약(${result.strengthStatus})·합충(${interTodayStr}) 근거로 구체적으로. 무엇에 집중하면 좋은지·어떤 방식이 맞는지·피하면 좋은 것·환경/시간 등 오늘에 맞는 포인트를 자유롭게 골라 풀고, 마지막은 오늘 실천할 작은 학습 행동 1개를 단정으로.`,
    '업무·일':     `오늘의 업무·일을 일진 십성·합충(${interTodayStr})·신강약(${result.strengthStatus}) 근거로 구체적으로(먼저 할 일·미룰 일·진행 방식·집중 시간대 등 오늘에 맞는 것 자유 선택). 직업 상태(${jobLabel || '일상'})는 단정 말고 일상 어휘로. 마지막 실천 행동 1개 단정.`,
    '창작·예술':   `오늘의 창작·예술을 식상(식신·상관)·일진 천간 십성·합충 근거로 구체적으로(영감 주제·매체, 작업 단계, 막히는 함정, 환경 등 자유 선택). 마지막 작은 창작 시도 1개 단정.`,
    '운동·체력':   `오늘의 운동을 신강약(${result.strengthStatus})·일진 천간/지지 오행(${todayGz.ganElement}/${todayGz.zhiElement})·12운성·합충 근거로 — 어떤 종목·강도가 맞는지 콕 집어 추천(운동이 많으니 좁혀줄 것)하고 피할 동작·부위도. 마지막 운동 약속 1개 단정.`,
    '건강·치료':   `오늘의 건강·치료를 신강약(${result.strengthStatus})·12운성·일진 합충·결핍오행(${zeroEls.length > 0 ? zeroEls.join('·') : '없음'}) 근거로 — 권장 페이스·살피면 좋은 몸의 신호·회피 행동·회복에 좋은 음식/환경 등 자유롭게. ★ 환자·치료 중 가능성 고려: 거창한 도전·신규 시도 금지, 만성질환/투병 어휘는 단정 회피하고 "컨디션·회복" 톤. 마지막 "무리 금지" 단정.`,
    '육아·돌봄':   `오늘의 육아·돌봄을 일진 십성(비견·식상·재성 등)·합 근거로 — 잘 통하는 활동·피하면 좋은 자극·부모 본인 컨디션 관리·가족 마찰 회피 등 자유롭게. 마지막 작은 약속 1개.`,
    '투자·재테크': `오늘의 재테크를 재성(편재·정재) 흐름·일진 충(${interTodayStr}) 근거로 — 진입/관망/정리 방향·회피 신호·점검 행동 등 자유롭게. 큰 결정은 일진 충 시 미루기. 마지막 안전 행동 1개.`,
    '인간관계':    `오늘의 인간관계를 일진 십성·합 근거로 — 잘 풀리는 만남·거리 둘 만한 패턴·연락 시점과 톤·회피 말투 등 자유롭게. 마지막 관계 행동 1개.`,
    '자기계발':    `오늘의 자기계발을 인성·식상 흐름·일진 십성 근거로 — 새 시도 vs 정리, 인풋 vs 아웃풋, 회피 패턴 등 자유롭게. 마지막 작은 시도 1개.`,
    '취미·여가':   `오늘의 취미·여가를 식상 흐름·일진 천간 십성·합 근거로 — 즐길 거리·혼자 vs 함께·회피 패턴·시간 등 자유롭게. ★ 즐기는 것에 죄책감 갖지 말 것(여가=다음 날 위한 충전). 마지막 작은 즐거움 약속 1개.`,
    '휴식·재충전': `오늘의 휴식·재충전을 신강약(${result.strengthStatus})·12운성·일진 십성(인성·식신 등) 근거로 — 휴식 형태·피하면 좋은 자극·회복에 좋은 시간대와 행동 등 자유롭게. ★ 새 시도·확장·고강도 운동 금지. 마지막 "푹 쉬어라" 단정.`,
  };
...
${ctx.customLoveState?.trim()
  ...
  : `★ 연애 미입력 — 이성 부분은 특정 연애 상황·상대를 지어내지 말고, 오늘 이성운의 일반 흐름(끌림·매력 발산·만남 가능성·관계 톤 등)으로만 풀이한다.`}

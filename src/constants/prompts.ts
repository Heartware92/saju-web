...
export interface RelationClassification {
  ...
  nuance: string;
  /** 라벨 결에 맞는 특수 섹션 제목 3~5개. 공통 7개와 섞여 본 풀이의 ▶ 섹션이 됨. */
...
- specialSections: 이 관계 결에서만 다뤄야 할 특수 섹션 제목 **3~5개**. 본 풀이는 아래 공통 7개 골격을 항상 포함하며, 거기에 너희가 정한 특수 섹션이 섞여 총 10~12개 ▶ 섹션이 된다. 공통과 겹치지 않게 라벨 고유의 결을 짚는 슬롯을 골라라.
  공통 7개(중복 금지): 핵심 요약 / 오행 상보 관계 / 마음의 결속·깊이 / 갈등·마찰 포인트 / 서로의 속마음 / 개운법·처방 / 이 관계의 미래·전망
  예 — "섹스파트너" 라면: ["본능적 갈증과 해소의 명리", "연인으로의 장기 전환 가능성", "비밀스러운 관계의 사회적 영향", "감정 과잉의 위험 신호"]
  예 — "원수" 라면: ["원한이 발화한 명리 지점", "관계 청산·화해 가능성", "주변 사람들이 보는 시선", "복수 충동의 위험"]
  예 — "돈 안 갚는 친구" 라면: ["우정과 채권이 충돌한 지점", "관계 청산 vs 회수 우선순위", "법적·심리적 대응 시기"]
...
아래 11개 ▶ 섹션을 정확히 이 순서대로 빠짐없이 작성하세요. 공통 7개 골격에 분류기가 미리 결정한 특수 ${c.specialSections.length}개가 흐름에 맞춰 끼워져 있습니다.

${(() => {
  // 공통 7 + 특수 끼우기: 핵심요약(공통1) → 오행상보(공통2) → 특수1 → 마음의 결속(공통3) →
  // 특수2 → 갈등·마찰(공통4) → 특수3 → 서로의 속마음(공통5) → 특수4(있으면) →
  // 개운법·처방(공통6) → 미래·전망(공통7) → 특수5(있으면, 미래 뒤 짧은 마무리로)
  const sp = c.specialSections;
  const list: { kind: 'common' | 'special'; title: string; guide: string }[] = [
    { kind: 'common',  title: '핵심 요약',          guide: `이 "${rawLabel}" 관계의 본질·점수 결을 한 호흡으로 선언. 일간 오행 관계·일지 음양합·용신 흐름 종합.` },
    { kind: 'common',  title: '오행 상보 관계',      guide: `두 사람 오행 분포로 결핍과 보완, 또는 충돌과 자극이 "${rawLabel}" 관계에서 어떻게 드러나는지.` },
    ...(sp[0] ? [{ kind: 'special' as const, title: sp[0], guide: `"${rawLabel}" 결에서만 다룰 수 있는 특수 슬롯. 명리 글자 인용 필수.` }] : []),
    { kind: 'common',  title: '마음의 결속·깊이',    guide: `왜 이 두 사람이 "${rawLabel}" 결로 묶이는가 — 끌림·악연·중독·운명적 매듭 어느 쪽이든 라벨 결로.` },
    ...(sp[1] ? [{ kind: 'special' as const, title: sp[1], guide: `라벨 특수 슬롯. 다른 섹션과 중복 금지.` }] : []),
    { kind: 'common',  title: '갈등·마찰 포인트',    guide: `충·형·신살·기신 충돌이 "${rawLabel}" 관계 특유의 어떤 장면에서 터지는지.` },
    ...(sp[2] ? [{ kind: 'special' as const, title: sp[2], guide: `라벨 특수 슬롯.` }] : []),
    { kind: 'common',  title: '서로의 속마음',       guide: `각자가 상대에게 말 못 하는 감정·진심을 1인칭 화법으로 대변. 십성 구조 근거.` },
    ...(sp[3] ? [{ kind: 'special' as const, title: sp[3], guide: `라벨 특수 슬롯.` }] : []),
    { kind: 'common',  title: '개운법·처방',         guide: `"${rawLabel}" 결을 존중하며 관계를 더 좋게 / 유지 / 정리하는 실용 처방 3~5가지.` },
    { kind: 'common',  title: '이 관계의 미래·전망', guide: `대운·세운 흐름과 라벨 결을 종합해 이 관계가 어디로 갈지 — 발전·전환·청산·화해 등 라벨에 맞는 결로.` },
    ...(sp[4] ? [{ kind: 'special' as const, title: sp[4], guide: `라벨 특수 마무리 슬롯.` }] : []),
  ];
  return list.map((s, i) => `${i + 1}. [${s.kind === 'common' ? '공통' : '특수'}] ▶ ${s.title}\n   ${s.guide}`).join('\n');
})()}

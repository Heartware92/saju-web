...
은유 제목 없이 본문만. 9개 영역(연애·일·재물·건강·학습·대인·횡재·멘탈·이동) 각각 "라벨 — 한 줄 풀이" 형식. 영역 사이 빈 줄 1줄로 9문단 분리, 줄 앞 기호·불릿 금지(라벨로 시작).
각 줄은 그 영역의 명리(일진 십성 ${todayGz.tenGodGan}/${todayGz.tenGodZhi}·합충·해당 궁·신살·신강약 ${result.strengthStatus})를 근거로 풀되, [today_scores] 점수와 톤이 일관되게(80+ 추진, 60~79 균형, 60 미만 주의). 9줄 중 절반 정도엔 구체 시각("오후 7시", "퇴근길") anchor를 넣으면 좋다. 표현·강조는 자유.
분량: 총 450~650자.
...
[today_sleep] — 오늘의 수면·회복 루틴
첫 줄: 은유 제목.
오늘 일진·용신(${yongSinElement})·결핍오행(${zeroEls.length > 0 ? zeroEls.join('·') : '없음'})을 근거로, 권장 취침/기상 시각(구체적으로)·잠들기 전 회피할 것·이완 루틴·침실 환경 중 오늘에 맞는 것을 골라 풀이한다. 직업(${jobLabel || '일상'})이 있으면 취침 패턴에 1회 자연 반영해도 좋다. 구성·강조는 자유.
분량: 240~340자.

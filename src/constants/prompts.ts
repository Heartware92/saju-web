...
· ★★★ 사용자 직접 입력 자연 호칭 변환 — 원형 박기 금지
  ...
  ★ 비표준 라벨은 관련 섹션([today_caution]·[today_strength]·[today_oneliner] 등)에도 의미로 1회씩 녹이되 직접 호칭 반복 금지.
...
★ 4축 동시 반영: 일진(오늘)만이 아니라 세운(올해 ${seWoonStr})·월운(이번 달 ${monthRunStr})의 결도 본문 톤·점수에 함께 작동시킨다. 예: 올해 세운이 큰 흐름(한 해의 테마)을, 월운이 이번 달 분위기를, 일진이 바로 오늘의 변동을 결정 — 같은 일진이라도 달·해가 다르면 풀이가 달라야 한다. today_basis·today_oneliner·today_fortune_message에서 4축이 어떻게 겹쳐 오늘이 되는지 1회 명시.
섹션별 반영 지침:

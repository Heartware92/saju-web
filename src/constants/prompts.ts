...
- 직업 상태: ${jobLabelOut}${hasJob ? '' : ' ★ 직업 미입력 — 직업/직장 장면·호칭을 절대 지어내지 말 것. today_persona_extra 는 직업 카드가 아니라 누구에게나 통하는 일반 행동 카드(오늘의 작은 시도)로 작성하고, 다른 섹션에서도 직업 호명 금지.'}
...
· ★★ 분량 상한·일관 (중요): 각 섹션의 명시 글자수 범위는 하한뿐 아니라 상한도 반드시 지킬 것. 일진 상호작용(충·합·형)이 많거나 풀이할 거리가 많은 날이라도 분량을 늘리지 말고, 가장 핵심인 작용만 골라 같은 분량 안에서 밀도 있게 쓸 것. 분량은 날짜·일진·시간대와 무관하게 매번 일정해야 한다 (같은 섹션을 다른 날에 봐도 글자 수가 1.5배씩 차이 나면 안 됨). 내용은 매일 달라지되 분량은 고정.
...
· ★★ 동적 분기 강제: [답변 자동 분류 결과](${allGroupsCode})에 매칭되는 그룹의 [섹션별 비중·강조점 분기 가이드]를 13개 본문 섹션(today_basis ~ today_fortune_message)에 모두 적용. 그룹이 여럿이면 누적 적용. 같은 사주라도 답변이 다르면 풀이가 확연히 달라야 한다. 분기 미적용 = 사고로 간주.
${hasJob ? `· ★★ 직업 맞춤 카드 강제: [today_persona_extra] 카드는 jobState=${jobLabel} 에 해당하는 [today_persona_extra 직업/상황 맞춤 포인트 카드] 가이드를 그대로 따름. 학생/직장인/자영업/구직중/주부 중 jobState 에 따라 카드 라벨·콘텐츠가 완전히 달라야 한다.
· ★★ 직업 상태 분기 강제: [직업 상태별 본문 톤·장면 분기 가이드]의 jobState=${jobLabel} 항목대로 본문 등장 장면을 그 직업 일상으로 맞춤. 답변 칩이 직업과 안 맞으면 jobState 우선으로 톤 변형 (예: 주부 + "회의·미팅" → "가족 모임 자리"로).` : `· ★★ 직업 미입력: today_persona_extra 는 직업 카드가 아니라 일반 행동 카드로 작성하고, 모든 섹션에서 특정 직업 호명·직장 장면을 만들지 말 것 (일상 보편 장면으로 처리).`}
...
첫 줄: 은유 제목.
본문 5요소 구조 (모두 포함, 자연스러운 문단으로). ★ 분량을 날짜·일진과 무관하게 일정하게 유지하기 위해, 각 요소의 문장 수를 아래 지정대로 지킬 것 (지정보다 줄여서 짧게 끝내지 말 것 — 모든 요소를 충분히 풀이):
1) 일진(${todayGz.gan}${todayGz.zhi}, ${todayGz.ganElement}·${todayGz.zhiElement}) 천간/지지의 의미 — 2문장.
2) 그것이 내 일간(${dayMaster}·${pillars.day.ganElement})·용신(${yongSinElement})·기신(${result.giSin})과 어떻게 만나는지 단정 — 2문장.
3) 4층 운기(대운·세운·월운·일진) 중 오늘 가장 강하게 작용하는 층 1개 지목 + 그 이유 — 2문장.
4) 일진×원국 합충(${interTodayStr}) 또는 일주 12운성(${pillars.day.twelveStage})이 오늘 어떻게 작용하는지 — 2문장.
5) 신강신약(${result.strengthStatus}) 관점에서 오늘이 일간에 유리/불리한지 + 한 줄 정리(오늘의 명리적 결) — 2문장.
★ 총 9~10문장, 본문 420~490자 (하한 420자 — 5요소를 모두 충실히 풀어 절대 짧게 끝내지 말 것 / 상한 490자 — 일진 상호작용(충·합·형)이 많은 날이라도 핵심 작용 위주로 추려 490자를 넘기지 말 것). 풍부함은 유지하되 날짜별 편차만 줄이는 것이 목표.
★ 분기 적용(${allGroupsCode}): [섹션 분기 가이드]의 today_basis 항목대로 강조점·관점만 변형 (분량은 위 420~490자 범위로 고정, 분기로 분량을 늘리거나 줄이지 말 것).
...
연애 — [일진 합·충 또는 정재/정관 흐름 1문장 + 시간 anchor 1개 또는 ${loveLabel || '일반 인간관계'} 호명 1회]
일·업무 — [일진 십성(${todayGz.tenGodGan}/${todayGz.tenGodZhi})과 ${jobLabel || '일상'} 직업 결합 + 시간 anchor]
...
★★★ 사용자 입력 처리 (today_timeflow):
  ...
  · 직업(${jobLabel || '일상'}) 일과 한 줄 — 사용자 진입 시간 구간 또는 다음 구간에 직업 호칭 1회 짧게 인용 (예: "직장인이라면 퇴근길에", "자영업자라면 오후 손님 응대 후"). 같은 직업 호칭은 본 섹션 안에서 1회만.
...
★★ 사용자 입력 처리 (today_sleep): 직업(${jobLabel || '일상'}) 일과 관점에서 취침 시각·이완 의식 중 1개를 직업 호칭 1회 짧게 변형 인용 (예: "야근이 잦은 직장인이라면 23시 전 디지털 디톡스부터", "프리랜서라면 자율 시간 끝을 24시로 못 박고"). q1/q2 답변 키워드 직접 노출 금지 — [절대 우선 룰] (D) 적용, 답변이 의미하는 회복·피로 결만 권장 취침 시각·이완 의식에 자연 반영 (답변 단어 자체는 본문에 등장 X).
...
★★ 사용자 입력 처리 (today_meal): 직업(${jobLabel || '일상'}) 식사 패턴을 식사 시간대·자리 톤(혼자/동료/가족) 결정에 1회 짧게 반영 (예: "직장인 점심은 동료 자리라 빠르게", "자영업자라면 손님 마감 후 19시쯤"). q1/q2 답변 키워드 직접 노출 금지 — [절대 우선 룰] (D) 적용, 답변이 의미하는 결만 식사 톤·시간대에 자연 반영 (답변 단어 자체는 본문에 등장 X).
...
${hasJob ? `${jobLabel} 상황과 무리 없이 어울리도록. ` : ''}운동 권장 시간대는 반드시 "지금 이후" 명시.
...
★★ 사용자 입력 처리 (today_exercise): 직업(${jobLabel || '일상'}) 종일 자세·체력 패턴이 운동 강도·종목 권장에 1회 자연 반영 (예: "앉아서 일하는 직장인이라면 허리 스트레칭 위주", "현장직이면 이미 체력 소모 → 가벼운 회복만"). q1/q2 답변 키워드 직접 노출 금지 — [절대 우선 룰] (D) 적용, 답변이 의미하는 컨디션·체력 결만 운동 강도 결정에 자연 반영 (답변 단어 자체는 본문에 등장 X).
...
★★ 사용자 입력 묻어남: q1/q2 답변(${q1Filled || '미답'} / ${q2Filled || '미답'}) 의 의미를 함정 측면으로 변형해 1회 인용 — 그 답변에서 파생될 수 있는 오늘의 함정 1줄 (예: "아쉬움" 답변 → "아쉬움이 자책으로 번지는 순간", "기대되는 일" 답변 → "기대가 조급함으로 변하는 순간"). 답변 직접 인용은 금지, 의미만 변형. 직업(${jobLabel || '일상'}) 함정 (실수·말실수·관계 갈등) 1줄도 자연 인용.
...
본문: 사용자 입력 상황(예: "건설업"·"공무원 준비"·"이직"·"창업" 등)에 실제로 맞는 5~6개 구체 행동 가이드를 LLM 자율 작성 — 일반 직장인 가이드 복붙 금지, 입력 직업/상황의 특수성 반영(현장 안전·체력 관리·자격증 시험·서류 준비·면접 등). 만세력(일진 ${todayGz.gan}${todayGz.zhi}·십성 ${todayGz.tenGodGan}/${todayGz.tenGodZhi}·합충·용신 ${yongSinElement})과 결합.`
  : hasJob
  ? `jobState=${jobLabel} 분기 적용:
첫 줄: jobState=${jobLabel} 에 해당하는 [today_persona_extra 직업/상황 맞춤 포인트 카드] 가이드의 카드 첫 줄 라벨을 그대로 또는 한국 자연 결로 짧게 변형해서 1줄 작성. (예: 학생 → "오늘의 학습 습관 한 가지", 주부 → "오늘의 나만의 시간")
본문: 위 [today_persona_extra] 가이드의 jobState=${jobLabel} 항목의 5~6개 콘텐츠 요소를 모두 포함해 자연스러운 3~4 문단으로 풀어쓰기. 만세력(일진 ${todayGz.gan}${todayGz.zhi}·십성 ${todayGz.tenGodGan}/${todayGz.tenGodZhi}·합충·용신 ${yongSinElement})과 결합.`
  : `직업 미입력 — 직업 카드가 아니라 일반 행동 카드로 작성 (특정 직업 호명·직장 장면 금지):
첫 줄: "오늘의 작은 시도 한 가지" 또는 한국 자연 결로 짧게 변형.
본문: 사용자 취미(${primaryHobby})·진입 시간대(${slotLabel})·오늘 일진(${todayGz.gan}${todayGz.zhi}·십성 ${todayGz.tenGodGan}/${todayGz.tenGodZhi}·용신 ${yongSinElement})에 기반해, 직업과 무관하게 누구에게나 적용되는 작은 실천 행동 5~6개를 자연스러운 3~4 문단으로 풀어쓰기.`
}
...
★ 사주아이·기존 서비스와 똑같은 표현(예: "차분한 직장 선배", "책상 위 서류 정리하기") 그대로 베끼지 말 것 — 사용자의 만세력 + 입력(${jobLabel || '일상'}·취미·답변)에 맞춤화.

[today_fortune_message] — 100~180자 (행운의 한마디 — 결과 페이지 최하단 마무리)
은유 제목 없이 본문만. 사주아이 "마지막 한마디" 패턴 — 다음 3요소 모두 포함:
1) 사용자 입력 1개 명시 호명 (예: "변호사로서", "야근으로 지친 오늘", "${jobLabel || '일상'} 인 당신") + 위로 1줄

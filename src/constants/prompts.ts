...
· 답변 키워드는 today_persona_extra 외 본문에 그대로 박지 말고 의미·결만 반영하되, 만세력 데이터(일진 ${todayGz.gan}${todayGz.zhi}·십성·합충·용신 ${yongSinElement}·신강신약 ${result.strengthStatus})와 결합해 풀이.`;
  ...
  const hobbyMethodGuide: Record<string, string> = {
    ...
    '공부·시험':   `오늘의 공부·시험을 일진 십성(${todayGz.tenGodGan}/${todayGz.tenGodZhi})·12운성·신강약(${result.strengthStatus})·합충(${interTodayStr}) 근거로 구체적으로. 무엇에 집중하면 좋은지·어떤 방식이 맞는지·피하면 좋은 것·환경/시간 등 오늘에 맞는 포인트를 자유롭게 골라 푼다.`,
    '업무·일':     `오늘의 업무·일을 일진 십성·합충(${interTodayStr})·신강약(${result.strengthStatus}) 근거로 구체적으로(먼저 할 일·미룰 일·진행 방식·집중 시간대 등 오늘에 맞는 것 자유 선택). 직업 상태(${jobLabel || '일상'})는 단정 말고 일상 어휘로.`,
    '창작·예술':   `오늘의 창작·예술을 식상(식신·상관)·일진 천간 십성·합충 근거로 구체적으로(영감 주제·매체, 작업 단계, 막히는 함정, 환경 등 자유 선택).`,
    '운동·체력':   `오늘의 운동을 신강약(${result.strengthStatus})·일진 천간/지지 오행(${todayGz.ganElement}/${todayGz.zhiElement})·12운성·합충 근거로 — 어떤 종목·강도가 맞는지 콕 집어 추천(운동이 많으니 좁혀줄 것)하고 피할 동작·부위도.`,
    '건강·치료':   `오늘의 건강·치료를 신강약(${result.strengthStatus})·12운성·일진 합충·결핍오행(${zeroEls.length > 0 ? zeroEls.join('·') : '없음'}) 근거로 — 권장 페이스·살피면 좋은 몸의 신호·회피 행동·회복에 좋은 음식/환경 등 자유롭게. ★ 환자·치료 중 가능성 고려: 거창한 도전·신규 시도 금지, 만성질환/투병 어휘는 단정 회피하고 "컨디션·회복" 톤. 무리하지 않는 결을 담되 매번 같은 문구로 닫지 말 것.`,
    '육아·돌봄':   `오늘의 육아·돌봄을 일진 십성(비견·식상·재성 등)·합 근거로 — 잘 통하는 활동·피하면 좋은 자극·부모 본인 컨디션 관리·가족 마찰 회피 등 자유롭게.`,
    '투자·재테크': `오늘의 재테크를 재성(편재·정재) 흐름·일진 충(${interTodayStr}) 근거로 — 진입/관망/정리 방향·회피 신호·점검 행동 등 자유롭게. 큰 결정은 일진 충 시 미루기.`,
    '인간관계':    `오늘의 인간관계를 일진 십성·합 근거로 — 잘 풀리는 만남·거리 둘 만한 패턴·연락 시점과 톤·회피 말투 등 자유롭게.`,
    '자기계발':    `오늘의 자기계발을 인성·식상 흐름·일진 십성 근거로 — 새 시도 vs 정리, 인풋 vs 아웃풋, 회피 패턴 등 자유롭게.`,
    '취미·여가':   `오늘의 취미·여가를 식상 흐름·일진 천간 십성·합 근거로 — 즐길 거리·혼자 vs 함께·회피 패턴·시간 등 자유롭게. ★ 즐기는 것에 죄책감 갖지 말 것(여가=다음 날 위한 충전).`,
    '휴식·재충전': `오늘의 휴식·재충전을 신강약(${result.strengthStatus})·12운성·일진 십성(인성·식신 등) 근거로 — 휴식 형태·피하면 좋은 자극·회복에 좋은 시간대와 행동 등 자유롭게. ★ 새 시도·확장·고강도 운동 금지.`,
  };
...
권장 강도·피할 동작·연결된 신체 부위 보호도 오늘에 맞게. 직업(${jobLabel || '일상'}) 자세·체력 패턴을 강도에 1회 반영해도 좋다. 구성 자유.
분량: 220~320자.
...
★ 본문은 반드시 "대인"과 "이성" 두 부분으로 또렷이 나눠 쓴다(각각 별도 문단, 사이 빈 줄). 두 부분이 내용상 분명히 구분되기만 하면 되고, 라벨 머리표("대인 —" 등)나 고정 도입은 강제하지 않는다 — 각 부분의 첫 문장·풀이 방식·구성은 자유(매일 다른 진입점).
  (1) 대인 — 가족·친구·동료 등 일반 인간관계. 오늘 일진 천간 ${todayGz.gan}(${todayGz.ganElement}·십성 ${todayGz.tenGodGan})과 내 사주의 상호작용(비겁·식상·관성 + 지지 합충 ${interTodayStr})을 근거로 잘 통하는 관계·마찰 유형·조심할 말투를 푼다.
...
${ctx.customLoveState?.trim()
  ? `★ 이성 부분에서 사용자 연애 상황 "${ctx.customLoveState}"을 자연 호칭으로 1회만 변환 인용(원문·따옴표 금지)하고, 그 상황에 맞는 권장 행동·조심할 점을 실용적으로. 도덕적 판단·훈계 절대 금지(특히 비표준 관계 — "정리하라/옳지 않다" 류 금지), 명리 흐름과 상황을 객관적으로 매칭만.`
  : ctx.loveState && ctx.loveState !== '공개 안 함'
  ? `★ 이성 부분에 연애 상태(${ctx.loveState})에 맞는 권장 행동 1개를 자연스럽게 포함.`

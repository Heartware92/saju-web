...
★ 단, 직업 미입력 시 "출근 직전·퇴근길·오후 미팅 후" 같은 직장인 전제 anchor 금지 — 특정 직업을 전제하지 않은 중립 시각 표현 사용(시각·행동 자체는 오늘 일진 흐름에 맞춰 구체적으로, 범용 표현으로 흐리지 말 것). 직업이 입력돼 있으면 그 직업 일과에 맞는 anchor로 자연 변형 가능.
...
★★ 시각·활동 다양화 (반드시 — 템플릿화 방지): 시각을 한 곳에 몰지 말고 오전·점심·오후·저녁·밤에 골고루 분산한다. 같은 영역을 매일 같은 시각·같은 활동으로 푸는 고정관념 금지 — 오늘 일진의 상호작용에 따라 시각·행동이 실제로 매일 달라져야 한다. 십성을 고정 활동에 1:1로 박지 말 것(특정 십성=특정 활동 식 금지) — 같은 십성도 그날 합충·궁·신강약에 따라 다른 장면으로.
...
★ 자율성 원칙 (가장 중요) — 각 구간에 '활동'을 못박지 말 것. "출근·오전 업무", "점심·오후 회의·외출", "퇴근·저녁 식사" 같은 특정 직업(직장인) 일과를 단정하지 말고, 그 시간대가 하루에서 갖는 의미(예: 아침=막 하루를 여는 시점, 한낮=하루 한복판, 저녁=하루를 닫아가는 시점)만 제시한 뒤, 오늘 일진 ${todayGz.gan}${todayGz.zhi}와 내 사주의 상호작용으로 "그 구간이 오늘 어떤 결·기운으로 흐르는지"를 자유롭게 풀이한다(무엇을 하라는 지시가 아니라 그 시간대의 오늘 결 읽기). 사용자 직업을 알면(현재: ${jobLabel || '미입력'}) 그 일과로 1회 자연 변형, 모르면 특정 직업을 전제하지 않은 장면으로(학생·자영업·주부·프리랜서·무직 누구에게나 맞되, 일반 상식이 아니라 오늘 일진×사주로 구체화) — 직장인 전제 어휘 금지. (아래 구간 설명의 괄호 예시는 '의미' 참고용이지 활동 지정이 아니다.)
...
★ "몇 시에 자라/일어나라" 식 고정 시각 룰 금지. 회복의 방향을 짚더라도 "오늘은 ~한 기운의 밤이라 ~할 때 회복이 잘 드는 흐름"처럼 사주적 설명으로 녹이고, 범용 루틴을 'to-do'로 권하지 말 것. 사용자가 본 시점(${slotLabel}) 이후의 회복 흐름으로 자연스럽게 잇는다. 직업(${jobLabel || '미입력'})을 알면 결에 1회 자연 반영. 구성·강조는 자유.
분량: 240~340자.
...
★ 오늘 일진상 당신 몸에 맞는 운동 종목 1~2개를 콕 집어 짚는다(종류가 많으니 좁혀줄 것 — 막연한 "가벼운 운동" 금지). 단 '일반 운동 권유'가 아니라 "오늘 당신 기운엔 ~한 움직임이 맞는 결"이라는 풀이로 — 왜 오늘 당신에게 그 종목인지가 사주로 드러나야 한다. 종목은 오늘 일진 천간 ${todayGz.gan}(${todayGz.ganElement})·지지 ${todayGz.zhi}(${todayGz.zhiElement})·십성(${todayGz.tenGodGan}/${todayGz.tenGodZhi})이 내 사주(신강신약 ${result.strengthStatus}·합충 ${interTodayStr})와 어떻게 만나는지로 정한다.
  일진 오행별 종목 방향(참고 — 그대로 베끼지 말고 신강약·합충으로 변주): 목=스트레칭·요가·등산·숲길 걷기 등 유연·뻗는 움직임 / 화=러닝·HIIT·댄스·구기 등 활기찬 유산소 / 토=코어·걷기·등산·정적 근력 등 안정·중심 운동 / 금=근력·필라테스·정돈된 반복 운동·호흡 / 수=수영·아쿠아로빅·가벼운 유산소·이완 스트레칭. (신약·기운 소모 답변이면 강도 낮춰 회복형으로, 신강·에너지 넘치면 발산형으로.)
강도가 어느 정도가 맞는 결인지·무리해지기 쉬운 동작·약해지는 신체 부위도 오늘 기운으로 읽어 녹인다(지시가 아니라 "오늘 당신 몸엔 ~한 결"). 직업(${jobLabel || '일상'}) 자세·체력 패턴을 1회 반영해도 좋다. 구성 자유.
분량: 220~320자.
...
첫 줄: 은유 제목.
오늘 합충(${interTodayStr})·흉성 신살(${sinSalBad})에서 비롯돼 "오늘 어떤 실수·함정이 생기기 쉬운 결인지, 멘탈이 어디서 흔들리는지"를 구체 장면으로 읽어준다(주의'사항 나열'이 아니라 그 흔들림이 사주상 왜 오늘 생기는지의 읽기). 대처는 그 결에서 자연히 따라오는 정도로만, 흉으로만 단정하지 말 것. ★ 닫는 문장 형태는 매일 자유 — 매번 같은 "~하라" 명령형으로 끝내지 말 것. 직업(${jobLabel || '일상'}) 관련 함정을 1줄 녹여도 좋다. 구성 자유.
분량: 260~360자.

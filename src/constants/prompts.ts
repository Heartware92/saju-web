...
· ★★★ 사용자 입력값 인용 — 카드별 분산 강제 (반복 금지)
  ...
  | 섹션 | customHobby | 직업 | 연애 | q1/q2 |
  |---|---|---|---|---|
  | today_basis | ✗ | ✗ | ✗ | ✗ (시간 호칭 1회만, 다른 입력 인용 금지) |
  | today_hobby_method | ● 필수 (N개 모두) | ✗ | ✗ | ● 필수 (1회 자연 인용 — 분야 활동과 답변 상태 연결) |
  | today_timeflow | ✗ | ● 진입 시간 구간에 직업 일과 1회 | ✗ | ● 필수 (진입 시간 구간에 답변 키워드 1회 자연 인용) |
  | today_sleep | ✗ | ● 야근·자율 시간 등 1회 | ✗ | △ 피로·휴식 답변시 1회 변형 |
  | today_meal | ✗ | ● 식사 시간·자리 톤 1회 | △ 가족·연인 식사시 | △ 식사·시간·사람 답변시 1회 변형 |
  | today_exercise | ✗ | ● 자세·체력 패턴 1회 | ✗ | △ 컨디션·체력 답변시 1회 변형 |
  | today_relationship | ✗ | ✗ | ● 필수 (자연 호칭 1회) | △ people 그룹시 답변 관계 유형 인용 |
  | today_caution | ✗ | ● 직업 함정 1줄 | △ 관계 함정 | ● 함정 측면으로 답변 의미 변형 1회 |
  | today_strength | △ (현재 ${primaryHobby} 연결 1줄) | △ | △ | ● 강점 측면으로 답변 의미 변형 1회 |
  | today_persona_extra | ✗ | ● 필수 (카드 첫줄·본문) | ✗ | ● (현재 1회 인용 룰 적용 중) |
  | today_lucky_card | ✗ | ✗ | ✗ | ✗ (메타 정보만) |
  | today_fortune_message | ✗ | ● 필수 (1회 호명) | △ 싱글 위로시 | △ 답변 위로 톤 |
...
본문: ${hobbyGuide}
${secondaryGuide}
${q1Filled || q2Filled ? `★ 사용자 답변 묻어남 강제: q1/q2 답변(${q1Filled ? `Q1 "${q1Filled}"` : ''}${q1Filled && q2Filled ? ' / ' : ''}${q2Filled ? `Q2 "${q2Filled}"` : ''}) 중 1개는 반드시 본문에 1회 인용. 직접 관련 있는 답변이 있으면 그 키워드를 1회 자연 노출하고 일진 ${todayGz.gan}${todayGz.zhi} 십성(${todayGz.tenGodGan}/${todayGz.tenGodZhi})과 어떻게 맞물리는지 1~2문장으로 풀이. 직접 관련 없어 보여도, 사용자가 답변한 상태를 분야 활동에 어떻게 녹일지 (예: "피곤한 상태라면 가볍게", "기대되는 일이 있다면 그 흐름을 살려서") 변형 인용 1회.` : '시간대 질문 답변이 없으므로, 입력하지 않은 부분을 추정해 만들지 말고 일반 가이드로.'}
...
★★★ 사용자 입력 묻어남 강제 (이 섹션이 사용자에게 "지금 내가 머무는 시간에 딱 맞네" 느끼게 하는 핵심):
  · q1/q2 직접 입력 답변(${q1Filled || '미답'} / ${q2Filled || '미답'}) 중 1개를 사용자 진입 시간(${slotLabel}) 구간에 자연스럽게 1회 인용. 답변의 키워드를 그 시간대 풀이 본문에 녹여서, 사용자가 "내가 적은 답변이 시간 흐름에 반영됐네" 라고 느낄 수 있게 작성. 답변이 미답이면 일반 풀이.
...
★★ 사용자 입력 묻어남: 직업(${ctx.customJobState || ctx.jobState}) 일과 관점에서 취침 시각·이완 의식 중 1개를 직업 호칭 1회 짧게 변형 인용 (예: "야근이 잦은 직장인이라면 23시 전 디지털 디톡스부터", "프리랜서라면 자율 시간 끝을 24시로 못 박고"). q1/q2 답변(${q1Filled || '미답'} / ${q2Filled || '미답'}) 이 피로·휴식 관련(쉬고 싶다·피곤·지친 등)이면 그 답변 키워드 1회 자연 변형 인용.
...
★★ 사용자 입력 묻어남: 직업(${ctx.customJobState || ctx.jobState}) 식사 패턴을 식사 시간대·자리 톤(혼자/동료/가족) 결정에 1회 짧게 반영 (예: "직장인 점심은 동료 자리라 빠르게", "자영업자라면 손님 마감 후 19시쯤"). q1/q2 답변(${q1Filled || '미답'} / ${q2Filled || '미답'}) 이 식사·시간·사람 관련이면 1회 변형 인용.
...
★★ 사용자 입력 묻어남: 직업(${ctx.customJobState || ctx.jobState}) 종일 자세·체력 패턴이 운동 강도·종목 권장에 1회 자연 반영 (예: "앉아서 일하는 직장인이라면 허리 스트레칭 위주", "현장직이면 이미 체력 소모 → 가벼운 회복만"). q1/q2 답변(${q1Filled || '미답'} / ${q2Filled || '미답'}) 이 컨디션·체력·피로 관련이면 그 키워드 1회 변형 인용.
...
[today_lucky_card] — 180~260자 (오늘의 행운 카드)
은유 제목 없이 본문만. 행운 메타데이터 4개를 한 카드로 묶어 풀이.
★ 문단 구분 필수: 각 항목 사이에 빈 줄 1줄을 반드시 넣어 4개 문단으로 분리. 한 덩어리로 뭉치지 말 것.
★ 형식 필수: 4개 문단 각 별도 줄로 "라벨 — 값 (1줄 풀이)" 구조. 줄 앞 기호·불릿 없이 라벨 자체로 시작:
컬러 — [한국어 색상명, 예: "딥블루", "은은한 핑크", "차콜그레이"] ([용신 ${yongSinElement} 또는 일진 ${todayGz.gan}${todayGz.zhi} 오행 근거 1줄])
숫자 — [1~9 중 1개 또는 두 자리] ([사주 숫자학 또는 일진 지지 ${todayGz.zhi} 근거 1줄])
아이템 — [구체 물건명, 예: "따뜻한 보온병", "작은 메모장", "패브릭 손수건"] ([오행 보강 또는 일진 호응 근거 1줄])
장소 — [구체 장소, 예: "조용한 카페 창가", "사무실 안쪽 책상", "공원 산책로"] ([일진 지지 ${todayGz.zhi}·${todayGz.zhiElement} 와 어울리는 공간 + 본 시점(${slotLabel}) 이후 갈 수 있는 곳 근거 1줄])
★ 줄 앞에 ▶·•·-·1) 같은 기호·불릿 일체 금지. 라벨 자체로 시작 ("컬러 — ..." 형태).
★ 모든 항목은 사용자 본 시점(${slotLabel}) 이후에 입거나 사용하거나 갈 수 있는 것만 (지나간 시간 가이드 금지).
...
★ 각 월 분량: 120~160자 (4~6문장), 줄바꿈 없이 이어서 작성.
...
export const generateTaekilAdvicePrompt = (
  saju: SajuResult,
  taekil: TaekilResult,
  /** 사용자가 100자 이내로 적은 행사 정황 (선택). 1·2·3위·조언·대체 방법에 반영 */
  detail?: string,
): string => {
...
- 수사일(受死日): 생기 끊김. 큰 행사 전반 흉.
- 월기일(月忌日): 음력 5·14·23일. 일반 행사 기피.
- 일파(日破): 본인 일주 지지 충일. 본인의 뿌리가 흔들림.
- 손없는날: 음력 일자 끝자리 9·0. 길일 보너스.
- 천화일(天火日): 화재·관재 상징. 입택·이사·신축 흉.
- 삼살일(三煞日): 그 해의 삼합 반대 방위 지지. 큰 행사·이동·결단 흉.
...
3) 신살명만 나열 금지. 그 신살이 사용자의 행사에 어떻게 작용하는지 1문장 설명.
...
4) ★★★★★ 각 [topN] 섹션은 반드시 다음 4줄 라벨을 한 줄씩 순서대로 출력. 4 라벨 누락 시 응답 무효:
   ...
   "조언: …" (6~9문장, 240~360자) — 이 날 무엇을 하면 좋은지 구체 행동을 ★ 2~3가지 분기 ★ 로 다양하게 제시.
     - 시간대 옵션 2개("오전 ○~○시에 ○하거나, 어렵다면 오후 ○~○시에 ○하기"),
     - 동선·방위 옵션 2개("주된 장소는 ○○ 방향이 좋고, 대안으로 ○○도 무방"),
     - 동행·복장·색상 옵션 등.
     반드시 "또는·만약·다른 방법으로·여건이 안 되면" 같은 분기 연결어로 대안 흐름을 표현.
     단순 한 가지 방법만 나열 금지. 사용자 정황을 1회 이상 반영.
   "주의: …" (3~5문장, 100~170자) — 이 날 조심해야 할 점. 약한 시간대·피할 행동·합충형 충돌 2~3가지를 명리 근거와 함께.
...
[작성 지침] 7개 섹션 구조 — 각 섹션은 반드시 [key] 마커 줄로 시작. 총 본문 1450~1900자. 5달 가치에 맞게 풍부하게.${result.hourUnknown ? '\n※ 시간미상이므로 자녀궁 직접 해석은 제한하고 연·월·일주 + 대운·세운 중심으로 풀이' : ''}
...
(빈 줄)
본문 200~260자 — 자녀복 경향을 한 줄로 단정(다자·소자·만득·귀한 자녀 한 명 중 하나)하고, 자녀성(남자=관성 정관·편관 / 여자=식상 식신·상관) 분포와 자녀궁(시주) 12운성·공망 여부 2~3가지를 같은 호흡에 자연스럽게 녹여서 명리 근거 제시. 이어서 자녀와의 인연 결 2~3문장(이른 자녀·늦은 자녀, 정이 깊은 결, 자녀로 인한 기쁨·책임의 방향 중 구체적으로)을 풀고 양면 묘사 1~2문장으로 마무리. 십성 용어는 일상어로 즉시 풀이. 추상 격언 금지.

[pregnancy]
본문 140~190자 — 자녀성 분포로 본 임신·출산 체질을 자연 임신 유리/시기 중요/의료 도움 권장 중 하나로 단정하고 명리 근거 1~2개를 같은 호흡에 녹임. 그 다음 임신·출산기에 본인이 보일 모습(체력·정서·태교 성향) 1문장과 양면 묘사(자녀복 풍성=양육 부담 양면 / 인연 박함=자유로움 양면) 1문장을 자연스러운 문단으로. 번호 매김 금지.

[temperament]
본문 130~180자 — ${result.hourUnknown ? '연·월·일주에서 유추한' : '자녀궁(시주) 12운성·지장간으로 본'} 자녀의 타고난 기질을 활동적/차분/예술적/학구적 중 하나로 단정하고 명리 근거 한 호흡, 일상 행동 묘사 2~3가지(놀이·식사·잠·친구관계 중 골라 구체 장면), 부모 입장에서 가장 인상적으로 느낄 자녀의 모습 1문장을 자연스러운 문단으로 풀어쓰기. 번호 매김 금지.

[parenting]
본문 170~220자 — 본인의 양육 성향을 인성형(보호·교육)/식상형(자유·표현)/관성형(규율·책임)/재성형(현실 감각) 중 하나로 단정하고 명리 근거 한 호흡. 본인의 양육 강점(자연스럽게 잘하는 부분) 1~2문장 구체 장면, 보완할 방향(의식적으로 보강해야 할 양육 태도) 1~2문장 구체 상황, 마지막에 부모-자녀가 가장 깊어지는 순간 1문장. 자연스러운 문단으로, 번호 매김 금지.

[compatibility]
본문 140~180자 — 일주 ${result.pillars.day.gan}${result.pillars.day.zhi} 기준 자녀와 잘 맞는 결(띠 또는 오행 2개 + 작용 메커니즘 + 어떤 상황에서 시너지)을 자연스러운 문장에 녹이고, 부딪치기 쉬운 결(띠 또는 오행 1개 + 갈등 해소법 구체 행동)을 이어서 풀어쓰기. 번호 매김 금지.

[career_hint]
본문 140~180자 — 자녀의 진로·재능 결을 식상형(예술·창작·표현)/인성형(학문·연구·교육)/관성형(리더십·공직)/재성형(사업·실리)/비겁형(독립·도전·운동) 중 하나로 단정하고 구체 분야명 2~3개를 자연스러운 문장에 녹임. 그 다음 자녀 어릴 때 부모가 어떻게 키워주면 좋은지 1~2문장. 번호 매김 금지.

[timing]
본문 200~270자 — 현재 대운과 올해 세운 자녀운 영향을 1~2문장 자연스러운 문단으로 풀고, 임신·출산 유리 시기 3~4개를 (연·월 + 명리 근거 한 줄) 형태로 본문에 녹임. 그 다음 실천 조언을 "- " 불릿 5개로 (1./2./3. 번호 절대 금지):
- 출산 유리한 계절 (이유 한 줄)
- 양육 시 주의할 점 (구체 행동)
- 자녀 교육 방향 (구체 분야·방식)
- 부모-자녀 관계 유의점 (구체 상황·대처)
- 임신·출산 준비 실천 1개 (구체 행동·환경)
마지막 한 줄(30~50자): 잠재력 응원.

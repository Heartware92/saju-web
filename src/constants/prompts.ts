...
[작성 지침] 850~1100자 내외 — 결제 콘텐츠이므로 풍성하게
...
1단락 — 제목 은유로 시작해 이 사람의 타고난 학업 체질 결론 (암기형/사고형/표현형/독학형 중 어떤 유형인지 격국·십성으로 단정). 십성 용어 첫 등장 시 같은 호흡 안에서 일상어로 즉시 풀어 설명.
2단락 — 인성·식상·관성 비율로 본 구체적 강점 영역 (필기시험/면접/논술/실기/자격증/학술연구 등) + 공부할 때 약한 점 1개. 시험 유형 적성 한 호흡 (객관식 강자=인성·식신, 논술·구술 강자=상관, 면접 강자=상관·도화, 실기 강자=편관·식상 결)
3단락 — 학업 관련 신살이 만드는 학습 패턴과 주의점 (문창→어학·글쓰기, 학당→정규 교육, 화개→연구·철학, 도화→집중력 저하·인간관계 에너지 소모, 역마→이동 학습형 등)
4단락 — **실전 학습 매칭 (신설)** — 다음 4가지 모두 구체적으로:
  · **공부 환경**: 혼자 깊이 vs 스터디그룹 vs 카페 vs 도서관 — 비겁(혼자형 vs 그룹형), 도화(주변 자극에 약함), 식상(말로 배우면 더 흡수) 결로 단정
  · **공부 시간대**: 아침·낮·저녁·새벽 중 어느 구간이 흡수율 높은지 — 12운성(${p.day.twelveStage})·용신(${result.yongSinElement}) 시간 기준 (예: "용신 ${result.yongSinElement}이 강해지는 ${result.yongSinElement === '목' ? '오전 5~9시' : result.yongSinElement === '화' ? '오전 11시~오후 3시' : result.yongSinElement === '토' ? '오후 1시~5시' : result.yongSinElement === '금' ? '오후 3시~7시' : '오후 9시~새벽 1시'} 구간이 사고가 가장 맑다")
  · **공부 방법**: 시각형(노트·다이어그램·필기) vs 청각형(인강·녹음·낭독) vs 토론형 vs 손으로 쓰는 형 — 인성(흡수형)·식상(표현형)·관성(반복형) 결로 매칭
  · **오행별 과목 강·약**: 목=어학·문학·국어, 화=예술·심리·미디어, 토=역사·지리·경영, 금=수학·논리·법학·과학(논리과목), 수=철학·연구·이공계(탐구과목). 강한 오행 = 자연스럽게 잡히는 과목 / 약한 오행 = 의식적 보강 필요. 구체 과목명 명시
5단락 — 현재 대운 + 올해 세운이 학업에 미치는 영향 + 구체적으로 유리한 달 2~3개 (월별 지지-인성/식상 관계 근거)
6단락 — 앞으로 3년 내 시험·자격·입학에 가장 좋은 시기 1개 (세운/대운 교차 근거)
7단락 — "- " 불릿 4개로 실전 공부 전략 (시험 직전 루틴 / 약한 과목 보완법 / 슬럼프 대처 / 경쟁 시험 vs 절대평가 중 유리한 유형)

★ 마지막 문장에서 제목 은유 회수 (앞에 쓴 자연 이미지 단어를 다시 한 번 가볍게 등장시켜 글을 닫기)`;
};

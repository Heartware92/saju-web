...
2) 마커 출력 순서: [top1] [top2]${topCount >= 3 ? ' [top3]' : ''}${worstDays.length > 0 ? ' [avoid]' : ''} [overall_advice] [alternative]
   누락·순서 변경 금지.
3) ★ 각 [topN] 섹션은 반드시 다음 4줄 라벨을 순서대로 출력:
   "종합: …" (4~6문장, 180~260자) — 이 날이 ${taekil.subItem ?? taekil.categoryLabel}에 적합한 명리적 결.
     일진 천간·지지 오행 + 원국 관계(생극·합충형) + 핵심 십성 영향 + 12운성 의미를
     자연스러운 문단으로 엮어 서술.
   "조언: …" (3~5문장, 120~180자) — 이 날 무엇을 하면 좋은지 구체 행동.
     시간대·방위·색상·동행자·동선 같은 실천 지침. 사용자 정황 반영.
   "주의: …" (2~4문장, 80~140자) — 이 날 조심해야 할 점. 약한 시간대·피할 행동.
   "키워드: " (4글자 이내 함축 표현 3개, 쉼표 구분. 예: "정인안정, 천덕길일, 수기조화")
4) [avoid] (흉일 있을 때만): 날짜 + 이유 2~3문장.
5) ★ [overall_advice] (200~280자) — "${taekil.subItem ?? taekil.customLabel ?? taekil.categoryLabel}에 대한 조언" 영역.
   - 1·2·3위 풀이를 종합해 ${taekil.subItem ?? taekil.customLabel ?? taekil.categoryLabel}이라는 행사 자체에 대한
     전반적 권고를 자연스러운 문단으로 작성.
   - 어떤 마음가짐·준비·태도가 도움 되는지, 어떤 함정이 있는지.
   - 특정 날짜에 매이지 않은 "이 행사에 대한 조언" 톤. 사용자 정황 반영.
6) ★ [alternative] (180~260자) — "추천 대체 방법" 영역.
   - 1·2·3위 날짜가 아니더라도 같은 행사를 더 잘 풀어낼 방법 1~3개.
   - 예: 시간대를 옮기기, 다른 분기·달 시도, 행사를 변형·분할, 동행자/장소 조정.
   - 추상 격언 금지. 구체 시점·행동 권고.
7) 추천일은 위 엔진 계산 결과에서만 고를 것 (top1·2·3). 임의 다른 날 추천 금지.
8) 용신·기신 언급 시 반드시 "오행(천간) — 십성" 형태로 쓸 것.
   (예: "용신인 화(병화·정화), 즉 편재가…")
9) 같은 표현 반복 금지. 각 날짜마다 다른 관점·어휘로 서술.
10) 총 ${isBirth ? '1500~2000' : '1300~1800'}자.
${METAPHOR_SHORT_GUIDE}

[taekil_advice]
출력은 [top1]·[top2]${topCount >= 3 ? '·[top3]' : ''} 각 섹션에 "종합:" / "조언:" / "주의:" / "키워드:" 4 라벨을 한 줄씩 사용하고, 마지막에 [overall_advice] 와 [alternative] 섹션을 작성하세요.${worstDays.length > 0 ? ' 흉일은 [avoid] 마커로 경고.' : ''}${isBirth ? ' 마지막에 출산 면책 문구 필수.' : ''}`;
};

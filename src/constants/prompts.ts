...
★ 첫 줄: 은유 제목 — 계절·자연·빛 이미지로 학업 체질 결을 한 줄 (**18자 이내**, 가벼운 부제 1개만)
★ 빈 줄
★ ★★★ **은유 중복 절대 금지** (가장 중요한 룰) ★★★
   본문 첫 문장은 제목과 **완전히 다른 단어·이미지**로 시작. 제목의 핵심 명사·형용사·동사를 본문 첫 줄에서 반복하면 즉시 사고.
   · BAD 예시 1: 제목 "고요한 겨울 강물, 그 아래 흐르는 지혜" → 본문 "고요한 겨울 강물처럼, 그 아래 흐르는 지혜..." (제목 그대로 복사 = 사고)
   · BAD 예시 2: 제목 "굳은 강물 아래 숨겨진 보석" → 본문 "굳은 강물 아래에서 빛나는 보석처럼..." (동일 핵심 단어 반복 = 사고)
   · GOOD 예시 1: 제목 "고요한 겨울 강물, 그 아래 흐르는 지혜" → 본문 "이 사주의 학업 결은 표면이 잔잔할수록 안쪽에서 깊어지는 흐름을 가진다."
   · GOOD 예시 2: 제목 "굳은 강물 아래 숨겨진 보석" → 본문 "겉으로는 더디고 정적이지만, 안쪽에는 시간이 쌓아 만든 단단한 결이 자리한다."
   → 본문은 제목 은유의 **분위기·결만** 가져가고, 등장 단어·구체 이미지는 **완전히 다르게** 다시 만들기.
   → 제목과 본문 첫 줄을 합쳐 같은 명사가 2개 이상 등장하면 사고. 같은 형용사·부사도 반복 금지.

★ 마지막 한 줄에서만 제목의 핵심 단어 1개를 가볍게 호명 (마지막 회수 — OK)

1단락 (80~110자) — 위 GOOD 예시 결로 시작해 학업 체질 한 줄 단정 (암기형/사고형/표현형/독학형 중 어떤 유형). 격국·십성 근거 1개만 노출하고 일상어 즉시 풀이. 추가 묘사 금지
...
[작성 지침] 본문 총량 **700~880자** (압축·핵심 위주. 줄글이 길게 늘어지지 않게, 구체 행동·시기·재능만)${result.hourUnknown ? ' — 시간미상이므로 자녀궁 직접 해석은 제한하고 연·월·일주 + 대운·세운 중심으로 풀이' : ''}
단락들은 자연스럽게 이어지되 각 단락은 짧고 핵심만. 늘어지는 묘사 금지. **단락 사이마다 빈 줄을 넣어 시각적으로 구분**시키기

★ 첫 줄: 은유 제목 (계절·자연·빛 이미지로 자녀복 결을 한 줄)
★ 빈 줄
★ 본문 첫 문장은 위 제목 은유의 **결을 이어받되, 같은 표현·핵심 단어를 그대로 반복 금지** — 다른 각도·시각으로 풀어내기 (예: 제목이 "고요한 겨울 강물, 그 아래 흐르는 지혜"이면 본문 첫 줄은 "얼어붙은 표면 아래 멈추지 않는 흐름이 ~" 식으로 같은 결의 다른 묘사). 별도 은유 도입 금지·동일 표현 반복 금지

1단락 (70~100자) — 제목 은유로 시작해 자녀복 경향 한 줄 단정 (다자/소자/만득/귀한 자녀 1명 중 하나). 명리 근거 1개만, 십성 용어는 같은 호흡 안에서 일상어 즉시 풀이

(빈 줄)

2단락 (80~110자) — 자녀성 분포로 본 임신·출산 체질 (자연 임신 유리·어려움·시기 중요 중 하나) + **양면 묘사** 한 줄 (자녀복 풍성=양육 부담 양면 / 인연 박함=자유 양면)

(빈 줄)

3단락 (60~90자) — ${result.hourUnknown ? '연·월·일주에서 유추한' : '자녀궁(시주) 12운성·지장간으로 본'} 자녀의 타고난 기질 (활동적/차분/예술적/학구적 중 단정), 묘사 1~2문장만

(빈 줄)

4단락 (180~220자) — **양육 가이드 (3가지 라벨 + 한 줄)** — 아래 형식 그대로 사용, 늘어지는 풀이 금지:

  · **양육 스타일**: (인성형·식상형·관성형·재성형 중 단정 한 줄) — (보완할 방향 한 줄)
  · **자녀와의 합·충**: (잘 맞는 띠/오행 한 줄) / (부딪치기 쉬운 띠 한 줄)
  · **자녀 진로 힌트**: (식상=예술·창작 / 인성=학문·교육 / 관성=리더십·공직 / 재성=사업·실리 / 비겁=독립·도전 중 단정, 구체 분야명 1~2개)

(빈 줄)

5단락 (60~90자) — 현재 대운 + 올해 세운이 자녀운에 미치는 영향 한 줄 (자녀성 강화·약화 판단)

(빈 줄)

6단락 (50~70자) — 임신·출산에 유리한 구체적 시기 2개 (월·년 + 근거 한 줄)

(빈 줄)

**이렇게 하면 도움돼요** (헤더 그대로 사용. "결론"·"종합"·"마무리" 금지)
- (출산 유리한 계절 한 줄)
- (양육 시 주의할 점 한 줄 — 구체 행동)
- (자녀 교육 방향 한 줄 — 구체 분야·방식)
- (부모-자녀 관계에서 유의할 점 한 줄)

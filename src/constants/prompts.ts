...
[작성 지침 — 동적 섹션 구조]
"${rawLabel}" 관계의 결(${c.nuance})에 맞춰 10~13개의 ▶ 섹션을 직접 구성해 작성하세요. 각 섹션은 반드시 두 줄 구조 — ▶ 메인 분야명 한 줄 + 다음 줄에 은유 부제목 한 줄.
...
★ 위 13개 주제 명칭("핵심 요약"·"갈등 포인트" 등)을 그대로 ▶ 메인 제목으로 베끼지 말 것. 각 주제를 "${rawLabel}" 맥락이 살아 있는 짧은 분야명(10~18자 한국어)으로 변환해 ▶ 메인 제목으로 출력하고, 다음 줄에 은유 부제목 한 줄을 단독 출력하라. 주제는 합치거나 분리해 10~13개 사이에서 조정 가능.

[섹션 출력 형식 예시 — 반드시 두 줄 구조]
▶ ${rawLabel} 관계의 본질
찰나의 불꽃, 서로를 태우는 유혹

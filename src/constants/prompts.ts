...
· ★★★ 사용자 입력값 인용 — 카드별 분산 강제 (반복 금지)
  ...
  ★★★ 사용자 입력 묻어남 매트릭스 — 실시간 운세의 핵심 목적은 "내가 적은 입력이 모든 섹션에 다 묻어나오네" 라는 유저 커스텀화 인지.
  아래 표대로 각 섹션에 입력을 분산 인용하되, 같은 입력은 카드별로 다른 측면(시간·행동·감정·환경·말투·강점·함정)으로 변형:

  | 섹션 | customHobby | 직업 | 연애 | q1/q2 답변 |
  |---|---|---|---|---|
  | today_basis | ✗ | ✗ | ✗ | ✗ (답변 키워드 노출 금지 — 명리 데이터만으로 풀이) |
  | today_hobby_method | ● 필수 (N개 모두) | ✗ | ✗ | △ 의미 결로만 흐름에 녹이기 (답변 키워드 직접 노출 금지) |
  | today_timeflow | ✗ | ● 진입 시간 구간에 직업 일과 1회 | ✗ | △ 시간대 결로만 변형 (답변 키워드 직접 노출 금지) |
  | today_sleep | ✗ | ● 야근·자율 시간 등 1회 | ✗ | ✗ (답변 키워드 노출 금지 — 회복 결만 자연 풀이) |
  | today_meal | ✗ | ● 식사 시간·자리 톤 1회 | △ 가족·연인 식사시 | ✗ (답변 키워드 노출 금지) |
  | today_exercise | ✗ | ● 자세·체력 패턴 1회 | ✗ | ✗ (답변 키워드 노출 금지 — 컨디션 결만 자연 풀이) |
  | today_relationship | ✗ | ✗ | ● 필수 (자연 호칭 1회) | ✗ (답변 키워드 노출 금지) |
  | today_caution | ✗ | ● 직업 함정 1줄 | △ 관계 함정 | △ 함정 측면 의미 변형 1회 (키워드 직접 노출 X) |
  | today_strength | △ (현재 ${primaryHobby} 연결 1줄) | △ | △ | △ 강점 측면 의미 변형 1회 (키워드 직접 노출 X) |
  | today_persona_extra | ✗ | ● 필수 (카드 첫줄·본문) | ✗ | ● ★ 직접 키워드 노출은 이 섹션 1곳에서만 — 본문 중간/끝에 1회 자연 변형 인용 |
  | today_lucky_card | ✗ | ✗ | ✗ | ✗ (메타 정보만) |
  | today_fortune_message | ✗ | ● 필수 (1회 호명) | △ 싱글 위로시 | ✗ (답변 키워드 노출 금지) |

  ★★★ 반복 인용 변형 룰 (위 매트릭스 적용 시 반드시 따를 것):
  (1) 같은 입력을 두 섹션 이상에 인용할 때 동일 문장 패턴 절대 금지.
      금지 예: "당신은 X 하셨으니" / "X 라고 하셨으니" / "X 인 당신께" — 같은 표현을 여러 카드에서 반복하면 사고로 간주.
  (2) 인용 방식 변형 — 같은 답변/입력을 카드별로 다른 측면으로 풀 것:
      · 시간 측면: "그 답변이 와닿는 건 오늘 ○○시쯤"
      · 행동 측면: "그 마음을 실제 행동으로 옮기려면 ○○부터"
      · 감정 측면: "그 감정이 강해질 때는 ○○를 해보세요"
      · 환경 측면: "그 답변에 어울리는 공간은 ○○"
      · 말투 측면: "그 답변 톤이 깊어질 때 ○○ 한 마디"
      · 강점·함정 측면: 같은 답변을 한 카드에선 강점, 다른 카드에선 함정으로 양면 풀이
  (3) 답변 원본 단어를 그대로 박지 말 것. 의미만 보존하고 자연 한국어로 변형. 따옴표 인용도 금지.
  (4) 같은 카드 내에서 같은 입력 2회 인용 금지. 한 카드 = 한 입력 한 번.

  ★★★★★ [절대 우선 룰 — 답변 반영 4축 시스템] ★★★★★
  ※ 이 블록은 위 매트릭스 및 아래 모든 섹션의 "★★ 사용자 입력 처리" 지시보다 우선한다.
  ※ 이 4축은 서로 다른 각도에서 같은 목표("사용자 답변이 자연스럽게 풀이에 녹는다")를 잡는 다차원 룰이다.
  ※ 단순 "금지" 만이 아니라 "어떻게 해야 하는지" 까지 함께 강제한다.

  ─────────────────────────────────────────────────
  ▣ 축 1 — 목표 재정의 (왜 인용하면 안 되는가)
  ─────────────────────────────────────────────────
  · "사용자 답변을 본문에 박아야 사용자가 만족한다" 는 발상은 잘못이다.
  · 사용자가 풀이에서 자기 답변 단어를 그대로 보면 → 오히려 인공적이고 어색하게 느낀다 ("내가 적은 거 그대로 받아 적은 거 아냐?").
  · 사용자가 자연스러움을 느끼는 순간은 → 자기 답변이 풀이의 "결·강도·시간대·톤" 에 녹아 있을 때 ("내가 적은 상태에 맞는 풀이를 받았네").
  · 따라서 작성의 목표는 **인용** 이 아니라 **반영** 이다. 답변 키워드를 본문에 박는 것이 아니라, 답변이 풀이의 형태를 결정하게 하는 것.

  ─────────────────────────────────────────────────
  ▣ 축 2 — 구조 강제 (각 섹션 첫 문장의 형태)
  ─────────────────────────────────────────────────
  ★ 모든 섹션의 첫 문장은 아래 3가지 패턴 중 하나로 시작해야 한다. 그 외 형태는 사고.

  (구조 A — 명리 데이터 시작) "오늘 일진 ${todayGz.gan}${todayGz.zhi} 의 ${todayGz.tenGodGan} 흐름이..." / "${todayGz.ganElement}·${todayGz.zhiElement} 두 기운이..." / "신강신약 ${result.strengthStatus} 사주에 오늘 일진 합충(${interTodayStr.slice(0, 30)}...) 이..."
  (구조 B — 섹션 본연 주제 시작) today_meal: "오늘 권장 음식은..." / today_sleep: "오늘 권장 취침 시각은..." / today_exercise: "오늘 권장 운동 강도는..." / today_relationship: "오늘 잘 통하는 만남은..."
  (구조 C — 시간·장면 시작) "오후 2시쯤 사무실에서..." / "퇴근길 6시 무렵..." (구체 시간+장소+행동 4요소 ≥2 충족 시 OK)

  ★ 위 3가지 외 첫 문장은 사고. 특히 답변 키워드를 첫 문장에 등장시키는 것은 절대 금지 (어떤 답변이든 — "쉬고 싶다", "아쉬움", "기대", "피곤", "즐거운" 등 사용자가 답한 단어 자체가 첫 문장에 등장하면 그 섹션 재작성).

  ─────────────────────────────────────────────────
  ▣ 축 3 — 반영 메커니즘 (답변이 어떻게 풀이에 녹는가)
  ─────────────────────────────────────────────────
  ★ 답변은 본문 키워드가 아니라 풀이의 다음 4가지 형태를 결정한다:

  (반영 1 — 강도 조절) 답변이 "회복·휴식·피곤·지친" 결이면 권장 강도를 약·저강도로, "기대·도전·즐거움" 결이면 중·강으로.
       예: today_exercise — 답변 결이 회복형이면 본문에 "스트레칭 10분만" (강도 조절). 답변 키워드는 본문에 등장 X.
  (반영 2 — 시간대 이동) 답변이 회복형이면 권장 시간대를 더 이르게·짧게 (예: 22시 취침). 추진형이면 늦게·길게.
  (반영 3 — 톤 조절) 답변이 부정 정서 결("아쉬움·후회·답답함")이면 단정·압박형 톤 회피, 따뜻한 위로 톤 우선. 긍정 결이면 추진·격려 톤.
  (반영 4 — 추천 종목/공간/음식의 결) 답변 결에 따라 권장 행동의 종류 자체가 바뀐다 (회복형: 따뜻한 차·산책·종이책 / 추진형: 새 시도·외출·근력).

  → 위 4가지 반영을 통해 사용자는 "내 상태에 맞춰 풀이가 왔다" 고 느낀다. 본문에 답변 키워드를 박지 않아도 자동으로 커스텀화 효과가 발생.

  ─────────────────────────────────────────────────
  ▣ 축 4 — 키워드 직접 노출 제한 + 자가 점검
  ─────────────────────────────────────────────────
  (제한 1) 답변 키워드 직접 노출은 today_persona_extra 1개 섹션에서만, 본문 중간/끝에서 1회. 따옴표 금지. Q1·Q2 동시 등장 금지 (둘 중 하나만).
  (제한 2) today_persona_extra 외 13섹션은 답변 키워드 본문 등장 0회.
  (제한 3) 같은 답변에서 파생된 풀이 결(예: "회복 우선" 톤)이 3개 이상 섹션에 동일 표현으로 등장 금지 — 축 3의 4가지 반영을 카드별로 다른 차원으로 분산.

  ★ 출력 직전 자가 점검 (3단계 — 한 단계라도 실패면 그 섹션 재작성):
    (점검 1) 각 섹션 첫 문장이 축 2의 구조 A/B/C 중 하나로 시작하는가? 답변 키워드로 시작한 섹션 있는가?
    (점검 2) today_persona_extra 외 섹션에 답변 키워드(사용자가 답한 단어 자체)가 본문에 등장한 곳 있는가?
    (점검 3) 같은 풀이 결 표현이 3개 이상 섹션에 반복 등장하는가? Q1·Q2 가 한 문장에 동시 등장하는 곳 있는가?

  ★ 답변이 미답이면 축 3 반영 적용 없이 일반 풀이로 진행 (추정·창작 금지).
...
★ 문단 구분 필수: 각 영역 사이에 빈 줄 1줄을 반드시 넣어 9개 문단으로 분리. 한 덩어리로 뭉치지 말 것.
★ 형식 필수 (9개 문단, 각 문단 50~100자, 줄 앞 기호·불릿 없이 라벨로 시작):
...
★ 줄 앞에 ▶·•·-·1) 같은 기호·불릿 일체 금지. 라벨 자체로 시작 ("연애 — ..." 형태).
...
본문: ${hobbyGuide}
${secondaryGuide}
${q1Filled || q2Filled ? `★ 사용자 답변 처리 (today_hobby_method): q1/q2 답변의 "키워드 직접 노출 금지" — 위 [절대 우선 룰 — 답변 인용 반복 사고 예방] 의 (D) 항목대로 답변이 의미하는 상태·결만 분야 활동 권장에 자연스럽게 녹일 것. 답변 키워드 자체를 본문에 박지 말고, "회복 결이 강한 상태에서는 가볍게", "조용히 몰입하고 싶은 결이면 짧은 단위로" 같이 결만 풀이. 따옴표 인용 절대 금지. 분야 활동을 일진 ${todayGz.gan}${todayGz.zhi} 십성(${todayGz.tenGodGan}/${todayGz.tenGodZhi})과 결합해 풀이하는 것은 핵심으로 유지.` : '시간대 질문 답변이 없으므로, 입력하지 않은 부분을 추정해 만들지 말고 일반 가이드로.'}
...
자정~새벽(현재) — 이 시간대의 기운 1~2문장. 지금이 새벽이라면 잠들 준비 또는 깊은 작업 시간.

아침(06~12시) — 출근·오전 업무 등 실전 장면으로 2~3문장.

오후(12~18시) — 점심·오후 회의·외출 장면으로 2~3문장.

저녁(18~24시) — 퇴근·저녁 식사·하루 마무리 장면으로 2~3문장.` : ''}${ctx.timeSlot === 'morning' ? `현재 오전이므로 "지금부터의 아침 + 오후 + 저녁 + 자정 전까지" 4구간 풀이 (320~440자). 새벽(00~05시) 풀이 금지:

아침(지금~12시) — 사용자가 지금 막 하루를 시작하는 시점. 오전 어떤 흐름이 좋고 어떤 일을 먼저 처리하면 좋은지 3~4문장 (가장 자세히).

오후(12~18시) — 점심·오후 흐름 2~3문장.

저녁(18~24시) — 퇴근·저녁 식사·하루 마무리 2~3문장.
...
오후(지금~18시) — 사용자가 지금 풀이 보는 시점. 점심 후 흐름, 오후 회의·작업·약속 등 실전 장면 3~4문장 (가장 자세히).

저녁(18~24시) — 퇴근 후·저녁 식사·하루 마무리 장면 2~3문장.
...
저녁(지금~22시) — 사용자가 지금 풀이 보는 시점. 저녁 식사·가족·휴식·정리 작업 등 실전 장면 3~4문장 (가장 자세히).

자정 전(22~24시) — 잠들기 전 마무리 행동·내일 준비 1줄·하루 회고 톤 2~3문장. today_sleep 섹션과 중복되지 않도록 "잠들기 직전까지 무엇을 할지"에 집중.` : ''}

★ 사용자 진입 시간(${slotLabel}) 구간이 가장 자세하고 (3~4문장), 나머지 미래 구간은 짧게 (2~3문장 이하). [today_flow] 점수와 일관.
★ 가장 집중이 잘 되거나 운이 강한 1구간 명시 + 이유 (일진 지지 ${todayGz.zhi} 와의 12지 관계 근거) — 단, 이미 지나간 시간을 운이 강했다고 풀이 금지.
★ 분기 적용(${allGroupsCode}): [섹션 분기 가이드]의 today_timeflow 항목대로 권장 활동·집중 시간 변형. 분량 ±15%.
★★★ 사용자 입력 처리 (today_timeflow):
  · q1/q2 답변의 "키워드 직접 노출 금지" — [절대 우선 룰] (D) 적용. 답변이 의미하는 시간 결·감정 결만 진입 시간 구간 풀이에 자연 통합 (예: 답변이 "쉬고 싶다"면 그 키워드 박지 말고 "이 시간대는 회복으로 끌고 가는 결이 맞아요" 같이 결로 풀이). 답변이 미답이면 일반 풀이.
  · 직업(${ctx.customJobState || ctx.jobState}) 일과 한 줄 — 사용자 진입 시간 구간 또는 다음 구간에 직업 호칭 1회 짧게 인용 (예: "직장인이라면 퇴근길에", "자영업자라면 오후 손님 응대 후"). 같은 직업 호칭은 본 섹션 안에서 1회만.
...
★★ 사용자 입력 처리 (today_sleep): 직업(${ctx.customJobState || ctx.jobState}) 일과 관점에서 취침 시각·이완 의식 중 1개를 직업 호칭 1회 짧게 변형 인용 (예: "야근이 잦은 직장인이라면 23시 전 디지털 디톡스부터", "프리랜서라면 자율 시간 끝을 24시로 못 박고"). q1/q2 답변 키워드 직접 노출 금지 — [절대 우선 룰] (D) 적용, 답변이 의미하는 회복·피로 결만 권장 취침 시각·이완 의식에 자연 반영 (답변 단어 자체는 본문에 등장 X).

[today_meal] — 분기 분량(아래 ★ 참조)
첫 줄: 은유 제목.
★ 현재 시점 이후 식사만 풀이 (timeSlot=${ctx.timeSlot}):
- midnight/morning: 아침·점심·저녁 모두 가이드 가능
- afternoon: 점심은 "이미 먹었다면" 짧게 회상만, 저녁·간식 위주
- evening: 저녁 식사·야식·자기 전 음료 위주 (아침·점심 가이드 금지)
본문 4요소 구조 (지나간 끼니 건너뛰기):
1) 용신(${yongSinElement})·결핍 오행(${zeroEls.length > 0 ? zeroEls.join('·') : '없음'})을 보강하는 추천 음식 2가지 — 맛·색·재료 구체적으로(예: "녹색 잎채소·신맛 — 봄나물 무침"). ★ 본 시점 이후 먹을 수 있는 끼니 기준.
2) 피해야 할 음식 1가지 + 이유(과다 오행 또는 자극 회피).
3) 권장 식사 시간대 1구간 + 식사 톤 1줄(혼자 조용히 / 가족과 / 천천히 30분). ★ 반드시 지금 이후 시간대.
4) 따뜻한 음료·차 1가지 추천(시간대 + 효능 1줄). ★ 본 시점 이후 음용 가능.
...
★★ 사용자 입력 처리 (today_meal): 직업(${ctx.customJobState || ctx.jobState}) 식사 패턴을 식사 시간대·자리 톤(혼자/동료/가족) 결정에 1회 짧게 반영 (예: "직장인 점심은 동료 자리라 빠르게", "자영업자라면 손님 마감 후 19시쯤"). q1/q2 답변 키워드 직접 노출 금지 — [절대 우선 룰] (D) 적용, 답변이 의미하는 결만 식사 톤·시간대에 자연 반영 (답변 단어 자체는 본문에 등장 X).

[today_exercise] — 분기 분량(아래 ★ 참조)
첫 줄: 은유 제목.
★ 현재 시점 이후 운동만 권장 (timeSlot=${ctx.timeSlot}):
- midnight/morning: 하루 중 운동 가능 시간대 모두 권장 가능
- afternoon: 오후·저녁 운동 위주 (아침 운동 권장 금지)
- evening: 저녁·야간 가벼운 운동만 (격한 운동은 수면 방해 → 자제)
본문 4요소 구조:
1) 오늘 남은 시간 운동 가능 여부·권장 강도(저강도/중간/고강도) — 신강신약(${result.strengthStatus})·일진 합충(${interTodayStr}) 근거. ★ evening 진입 시 고강도 권장 금지.
2) 추천 종목 1~2가지 + 분량(예: "스트레칭 10분 + 산책 20분", "근력 운동 40분"). ★ 지금 이후 가능한 종목.
3) 피해야 할 동작·종목 1가지 + 이유(오행 결핍·과다 + 일진 충 근거).
4) 일진 ${todayGz.zhiElement} 오행이 신체 어느 부위(간·심·비·폐·신)와 연결되는지 + 그 부위 보호 행동 1줄.
${ctx.jobState} 상황과 무리 없이 어울리도록. 운동 권장 시간대는 반드시 "지금 이후" 명시.
...
★★ 사용자 입력 처리 (today_exercise): 직업(${ctx.customJobState || ctx.jobState}) 종일 자세·체력 패턴이 운동 강도·종목 권장에 1회 자연 반영 (예: "앉아서 일하는 직장인이라면 허리 스트레칭 위주", "현장직이면 이미 체력 소모 → 가벼운 회복만"). q1/q2 답변 키워드 직접 노출 금지 — [절대 우선 룰] (D) 적용, 답변이 의미하는 컨디션·체력 결만 운동 강도 결정에 자연 반영 (답변 단어 자체는 본문에 등장 X).
...
★ 분기 적용(${allGroupsCode}): [섹션 분기 가이드]의 today_relationship 항목대로 변형. people 그룹이면 분량 +30% + 답변이 의미하는 관계 결(가족/친구/연인/동료 중)을 본문에 반영하되 답변 키워드 직접 노출 금지 — [절대 우선 룰] (D) 적용. 기본 240~320자.
...
★★ 사용자 입력 묻어남: q1/q2 답변(${q1Filled || '미답'} / ${q2Filled || '미답'}) 의 의미를 함정 측면으로 변형해 1회 인용 — 그 답변에서 파생될 수 있는 오늘의 함정 1줄 (예: "아쉬움" 답변 → "아쉬움이 자책으로 번지는 순간", "기대되는 일" 답변 → "기대가 조급함으로 변하는 순간"). 답변 직접 인용은 금지, 의미만 변형. 직업(${ctx.customJobState || ctx.jobState}) 함정 (실수·말실수·관계 갈등) 1줄도 자연 인용.
...
★★ 사용자 입력 묻어남: q1/q2 답변(${q1Filled || '미답'} / ${q2Filled || '미답'}) 의 의미를 강점 측면으로 변형해 1회 인용 — 그 답변에서 살릴 수 있는 오늘의 강점 1줄 (예: "기대되는 일" → "그 기대를 추진력으로 쓰기 좋은 시간", "푹 쉬고 싶다" → "오늘은 쉬는 것 자체가 가장 큰 운영 전략"). 답변 직접 인용은 금지, 의미만 변형.
...
[today_lucky_card] — 140~220자 (오늘의 행운 카드)
은유 제목 없이 본문만.
★ 색상·숫자·방위·시간대·보석·활동은 위 시각 카드(LuckyVisualCard)에 결정값으로 이미 표시되므로 본문에서 절대 반복·언급 금지. 본문은 시각 카드에 없는 행운 아이템·장소 2가지만 풀이한다 (색·숫자를 본문에 적으면 카드와 모순 발생 — 엄금).
★ 문단 구분 필수: 아이템 문단 / 장소 문단 2개로 분리하고 사이에 빈 줄 1줄.
★ "라벨 — 값" 형식·줄 앞 기호·불릿(▶·•·-·1) 등) 일체 금지. 자연스러운 서술 문장으로만 작성.
아이템: 본 시점(${slotLabel}) 이후 챙기면 좋은 구체 물건 1개(예: 따뜻한 보온병·작은 메모장·패브릭 손수건)를, 일진 ${todayGz.gan}${todayGz.zhi} 오행 호응 근거와 함께 한 문단으로 풀이.
장소: 본 시점(${slotLabel}) 이후 갈 수 있는 구체 장소 1곳(예: 조용한 카페 창가·공원 산책로)을, 일진 지지 ${todayGz.zhi}·${todayGz.zhiElement} 와 어울리는 근거와 함께 한 문단으로 풀이.
★ 모든 항목은 사용자 본 시점(${slotLabel}) 이후에 사용하거나 갈 수 있는 것만 (지나간 시간 가이드 금지).
★ 사주아이·기존 서비스와 똑같은 표현(예: "차분한 직장 선배", "책상 위 서류 정리하기") 그대로 베끼지 말 것 — 사용자의 만세력 + 입력(${ctx.customJobState || ctx.jobState}·취미·답변)에 맞춤화.
...
★ 이모지·이모티콘·특수 기호 일체 금지 (분위기는 문장 자체로만 전달).
...
export const ZAMIDUSU_SECTION_KEYS = [
  ...
  'body_palace',  // 신궁 (단독)
  'relations',    // 부처·자녀·형제·노복·부모 (5개 관계궁)
  'wealth',       // 재백·관록·전택 (3개 재물·일)
  'body_mind',    // 질액·복덕·천이 (3개 몸·마음·이동)
  'mutagen',      // 사화 (4개 균등 + 상호작용)
  'interactions', // 합·충·삼방사정
  'daehan',       // 대한 10년 리듬 + 비행 사화
  'sohan',        // 소한 12년 순환
  'advice',       // 별의 조언 (6개 실천)
] as const;
...
export const ZAMIDUSU_SECTION_LABELS: Record<ZamidusuSectionKey, string> = {
  ...
  body_mind:    '몸과 마음의 하늘',
  mutagen:      '사화 — 별의 변주',
  interactions: '별과 별의 만남 — 합·충·삼방사정',
  daehan:       '대한 — 10년 리듬',
  sohan:        '소한 — 한 해의 흐름',
  advice:       '별이 건네는 조언',
};
...
[relations] — 관계 5궁 (560~720자)
첫 줄: 은유 제목 (관계의 깊이·갈등 양상을 자연 이미지 대비로)
본문: 부처궁·자녀궁·형제궁·노복궁·부모궁 다섯 방.
각 궁마다 다음을 한 문장씩 (총 5개 미니 단락):
- 부처궁: 좌한 별의 이름·키워드 명시 + 끌리는 이성 성향
- 자녀궁: 좌한 별 명시 + 자녀 복 유형
- 형제궁: 좌한 별 명시 + 형제와의 거리감
- 노복궁: 좌한 별 명시 + 친구·후배 복
- 부모궁: 좌한 별 명시 + 부모 인연 깊이
+ 대궁(對宮) 영향: 부처궁 ↔ 관록궁이 충돌·조화하면 직장-결혼 균형 한 줄
+ 마지막에 갈등 가능 포인트 1개 + 관계 복 유형 1개로 종합

[wealth] — 재물·일 (520~680자)
첫 줄: 은유 제목 (재물 흐름·커리어 모양을 자연 이미지 대비로)
본문: 재백궁·관록궁·전택궁 세 방.
- 재백궁: 좌한 별 명시 + 수입 스타일을 이미지(달빛처럼 쌓이는 돈 vs 혜성처럼 들어왔다 빠지는 돈)로 묘사
- 관록궁: 좌한 별 명시 + 적합 직군 2~3개 (별의 성격에 근거 — 자미=리더십·천기=기획·무곡=재무·염정=관리 등) + 승진·이직 흐름
- 전택궁: 좌한 별 명시 + 부동산 축적 패턴 + 첫 집·큰 자산 마련 시기 단서
- 재물 함정 1개 (별·사화 근거 명시)
- 권할 재물 행동 1개

[body_mind] — 몸·마음·이동 (480~620자)
첫 줄: 은유 제목 (약한 곳·회복 방식을 자연 이미지 대비로)
본문: 질액궁·복덕궁·천이궁 묶어 풀이.
- 질액궁: 좌한 별의 오행으로 취약 장부 1~2개 (목=간담·화=심장·토=비위·금=폐·수=신장) + 어느 계절·시기 주의
- 복덕궁: 좌한 별 명시 + 스트레스 쌓이는 방식 + 회복 취미·환경 1개
- 천이궁: 좌한 별 명시 + 해외·이사·이민 길흉 + 유리한 방향(지지 ↔ 방위 매핑)
- 정신 건강 신호 1개 + 대응법 1개
- 마음에 쉼이 필요한 순간 묘사 1문장

[mutagen] — 사화의 변주 (700~900자)
첫 줄: 은유 제목 (별이 다른 노래를 부르는 이미지)
본문: 화록·화권·화과·화기 4개를 균등하게(각 150~200자) 깊이 풀이.
...
+ 사화 간 상호작용 한 단락: 화록과 화기가 같은 궁/대궁에 있으면 "복 뒤의 함정" / 화권과 화과가 어울리면 "권세에 명예가 따름" 등
+ 마지막 문장: 4개 사화의 균형이 인생에 어떤 톤을 주는지 + 은유 회수

[interactions] — 별과 별의 만남 — 합·충·삼방사정 (400~550자)
첫 줄: 은유 제목 (별의 만남·충돌을 이미지로. 예: "마주 보는 두 별, 부딪치는 빛")
본문: 자미두수에서 가장 중요한 별 관계 — 삼방사정(三方四正).
- 명궁 기준 삼방(재백궁·관록궁) + 대궁(천이궁) 세 궁이 명궁에 어떻게 비춰 들어오는지
- 가장 인생 무게가 실리는 축 한 문장 (예: "명궁-재백궁-관록궁 삼각형이 활성화 — 일과 돈이 인생의 중심")
- 화록·화기가 합쳐서 만드는 길흉 별·궁 1~2개 짚기
- 형충(刑沖)·궁간(宮干) 관계 — 직접 충돌하는 궁 쌍이 있으면 어디서 갈등이 터지는지
- 마무리: 이 명반의 가장 큰 시너지 1개 + 가장 큰 위험 1개
...
[sohan] — 소한 12년 순환 (300~420자)
첫 줄: 은유 제목 (12년 주기의 별 순환 이미지)
본문: 소한(小限) — 매년 1궁씩 시계방향 회전하는 연 단위 운세 흐름.
- 소한이 무엇인지 한 줄로 쉽게 (12년 순환 / 한 살씩 한 궁 / 그 해의 무대 조명)
- 현재 나이의 소한 궁 명시 (palaces[i].ages 에서 현재 나이 가까운 것)
- 그 궁의 좌한 별이 이 해의 핵심 분위기를 어떻게 만드는지
- 다음 3년의 소한 흐름 (3개 궁 순회) 한 문장씩
- 대한과 소한이 만나면 강조되는 영역 한 문장
...
export const NAME_SECTION_KEYS = [
  ...
  'strength',   // 살아나는 면 — 이름이 받쳐주는 영역
  'shadow',     // 놓치기 쉬운 그늘 — 약점·주의 (좋은 말 금지)
  'preserve',   // 현 이름 살리는 법 — 개명 없이 보완
...
[작성 지침] 7개 섹션 구조 — 각 섹션은 반드시 [key] 마커 줄로 시작. 총 본문 ${isHanjaMode ? '2200~2800' : '1700~2100'}자. 5달 가치에 맞게 풍부하게.
- 각 섹션 본문은 명시된 최소치 미만으로 작성 금지 (미만이면 양면 묘사·구체 장면 추가해 채울 것).
...
(빈 줄)
본문 280~380자:
  · 첫 문장 단정: "이 이름은 사주를 [보강 / 중립 / 거스름] 합니다." 셋 중 하나로 단정. (헤지 금지)
  · 4축(음령·${isHanjaMode ? '자원·수리오행·' : ''}81수리) 중 가장 강한 1축과 가장 약한 1축을 한 줄씩 단정 + 각 축이 사주 어디와 맞물리는지 한 호흡으로 근거 명시.
  · 일상에서 어떻게 발현되는지 또는 어떤 시기·영역에서 빛나는지 2~3문장으로 구체 장면 묘사 (직장·인간관계·결정 순간 중 하나).
  · 마지막 한 줄: 이 이름과 함께 살아가는 사람이 자기다움을 가장 분명히 느끼는 순간 한 호흡.
...
[four_axis]  — 4축 진단 (객관적 분석, 강·약 모두)
본문 ${isHanjaMode ? '560~720' : '340~440'}자:
  ① 음령오행 — ${koreanInitialsElements.join('·') || '(분석 불가)'} 분포가 사주(용신 ${yongSinEl}·기신 ${result.giSin}·신강신약 ${result.strengthStatus})와 어떻게 맞물리는지 단정 2~3문장 (어느 초성이 어느 오행을 어디서 보태는지·약점도 함께).
${isHanjaMode
  ? `  ② 자원오행 — 한자 부수가 어떤 오행을 보태고 어디가 부족·중복인지 단정 2~3문장 (각 한자별 자원오행 + 사주에 어떤 영향).
  ③ 수리오행 — 4격 각 수의 끝자리 오행이 사주 용신과 매칭되는지 단정 2~3문장 (예: "원격 수리오행이 용신 ${yongSinEl}과 일치해 초년 기반이 자연스럽게 받쳐줍니다"). 어느 격에서 빛나고 어느 격이 약한지 함께.
  ④ 81수리 4격 길흉 — 원격→형격→이격→정격을 각각 "[숫자]수 [등급]" 한 번 인용 + 결 1~2문장 (총 6~8문장). 격마다 인생 시기·영역에서 어떻게 발현되는지 구체 묘사.
  마지막 한 줄: "4축 종합으로 가장 강한 축은 ○○이고, 가장 약한 축은 △△입니다." 단정.`
  : `  ② 자원오행 — 한자 정보 미입력으로 분석 불가. 한 문장.
  ③ 수리오행 — 한자 정보 미입력으로 분석 불가. 한 문장.
  ④ 81수리 4격 — 한자 획수 없이는 계산 불가. 한 문장.
  마지막 한 줄: "현재 4축 중 음령오행 1축만 분석되어 종합 판정은 제한적입니다. 한자를 알려주시면 나머지 3축까지 봐드려요."`}

[strength]  — 살아나는 면 (이 이름의 강점)
본문 300~400자:
  · 이 이름이 받쳐주는 영역(커리어/관계/건강/재물/공부/창의 등) 2~3개를 단정으로 지목.
  · 각 영역을 받치는 4축 근거를 한 줄씩 (예: "음령에 용신 ○이 있어 ~~ 영역에서 자연스럽게 흐름이 만들어집니다") — 2~3개 근거를 자연스러운 문단에 녹임.
...
본문 300~420자.
  ...
  · 이 이름의 약한 면 2~3개. 4축 중 어디서 비롯되는지 근거 명시.
...
마지막 한 줄: 이 보완으로도 본인이 살리지 못한 결은 다음 안내(개명 고려)에서 짚는다는 한 호흡으로 마무리.
  ★ 절대 금지: 본문에 "[rename]" "[preserve]" 같은 대괄호 영문 마커를 직접 적지 말 것. 마커는 섹션 시작 줄에만 사용. 본문에서 다음 섹션을 가리킬 때는 "다음 안내" "아래 개명 항목" 같은 한국어로만 표현.
...
export const generateDreamInterpretationPrompt = (dreamText: string): string => {
  ...
  return `당신은 한국 전통 주공해몽·민속 해몽 35년 전문가이면서, 동시에 프로이트 정신분석·융 분석심리학에도 능통한 꿈 해석가입니다. 사용자의 꿈을 두 가지 관점(동양식·서양식)에서 풀어주세요. (사주·생년월일은 사용하지 않습니다. 꿈 자체만으로 해석합니다.)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[★★★★★ 출력 형식 — 반드시 준수, 어기면 응답 무효]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
출력은 반드시 다음 5개 마커 섹션으로 구성. 순서 변경·누락 절대 금지.

★ 가장 첫 줄은 [diagnosis] 한 줄. 다른 텍스트·인사·서두 금지.
★ 6개 마커 모두 빠짐없이 출력. 누락 시 응답 무효.
★ 모든 마커는 대괄호와 영문 원형 그대로. 한글 라벨 치환 금지.

[diagnosis]
(짧은 진단 — [진단 작성 지침])

[symbols]
(꿈 속 상징 3~5개 — [상징 작성 지침])

[oriental_interpretation]
(동양식 본문 — [동양식 작성 지침])
...
[입력 모드 인식 — 매우 중요]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
사용자 입력은 두 형식 중 하나입니다.

A) 선명 모드 — 자유 서술 문장 ("연예인 수지와 성관계를 했다…").
B) 흐릿 모드 — 라벨된 항목 묶음. 다음 라벨이 보이면 흐릿 모드:
   "등장물:", "행동:", "감정:", "꾼 시간대:", "반복해서 꾸는 꿈", "추가 기억:"
   이 경우 사용자는 또렷이 기억나지 않아 조각만 적은 것이므로:
   - 적힌 단어를 반드시 본문에 그대로 인용 (예: "등장물에 '수지'를 적으셨어요", "감정에 '설렘'을 골라주셨어요").
   - 사용자가 적지 않은 인물·장면 임의 추가 금지. 시나리오 창작 금지.
   - 단정 톤 줄이고 "~일 가능성이 있어요" 보수적 풀이 더 자주.
   - 입력이 매우 짧으면 [advice] 본문 끝에 "또렷이 기억나는 장면이 더 떠오르면 추가해서 다시 물어보시면 더 정확해요" 한 문장.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[공통 규칙]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- Markdown(#, ##, **, \`\`, >) 절대 금지. 이모지 금지. AI 티 표현 금지.
- 구어체 "~합니다/~예요". 단정 톤 (단 흐릿 모드는 보수 톤 더 자주 허용).
- 단락 사이 빈 줄 1줄. 한 단락 평균 3~5문장.
- ★ 여섯 섹션 모두 사용자가 적은 장면·인물·감정·라벨 항목을 직접 인용. 여섯 섹션 각각 최소 1회씩 인용 의무.
- 사용자가 인물 이름을 적었으면 본문에 그 이름을 최소 1회 그대로 사용.
- 여섯 섹션은 서로 다른 어휘·관점. 같은 결론·표현 반복 금지.
- 사람을 놀라게 하는 단정 표현(예: "큰 화", "사망의 전조") 절대 금지. 우려는 "한 번 점검할 시점" 식으로.
- 사용자의 꿈에 등장하지 않은 인물·장면을 임의 추가 금지.
...
첫 줄: 짧은 태그를 " · " 로 연결. 형식: "길흉판정 · 꿈종류[ · 시점단서]"
  예) "길몽 · 심리몽 · 새벽몽" / "혼재몽 · 예지몽" / "흉몽 · 계몽 · 반복몽"
  - 길흉: 길몽 / 흉몽 / 혼재몽 중 1개.
  - 꿈종류: [꿈 종류 체크리스트] 중 가장 부합하는 1개.
  - 시점단서: 입력에 시간 라벨이 있을 때만.

둘째 줄: 진단 근거 1~2문장. 사용자 입력 라벨·단어를 1회 이상 인용.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[상징 작성 지침 — symbols]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
총 3~5개 상징. 각 상징을 한 줄에 다음 형식으로:

상징명=전통의미 / 현대의미

- 상징명: 사용자가 적은 단어 그대로 (예: "수지", "성관계", "기쁨", "바다", "도망"). 가공 금지.
- 전통의미: 주공해몽·한국 민속 관점 20~40자.
- 슬래시 1개 " / " 로 구분.
- 현대의미: 정신분석·일상 심리 관점 20~40자.
- 줄바꿈으로 구분. 들여쓰기·빈 줄 금지.
- 반드시 3개 이상 5개 이하. 모두 사용자 입력에 등장한 단어.
- 흐릿 모드면 "등장물", "행동", "감정" 라벨에 적힌 단어를 우선 선택.

예:
수지=전통: 명예·인기·사회적 인정의 상징 / 현대: 자아 이상(Ego Ideal)의 투영
성관계=전통: 깊은 결합·생명력의 응축 / 현대: 무의식 통합의 신호
기쁨=전통: 길몽 방향을 강화하는 핵심 감정 / 현대: 긍정 정서의 표면화

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[동양식 작성 지침 — oriental_interpretation]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
총 700~950자. 주공해몽·한국 민속·도교 음양오행 관점.

1단락 — 첫 줄에 은유 제목 1줄 (대비되는 두 자연 이미지, 7~16자, 예: "흐린 강 너머, 떠오르는 새벽"). 다음 줄부터 본문 시작.

2단락 — 매칭된 상징 2~3개의 전통 의미를 주공해몽·민속 톤으로 풀이. 사용자가 적은 장면을 그대로 한 번 되짚기. 상징의 [길/흉/혼재] 폴라리티 명시. 음양오행 기운 흐름(예: "양의 기운이 응축된 결") 한 문장.

3단락 — [맥락 규칙]("보았다 vs 품었다 vs 쫓겼다")과 [감정 규칙]("따뜻함은 상징을 길몽으로 전환")이 해석을 어떻게 조정하는지. 감정이 상징과 어긋나면 "전통적으로 감정이 상징보다 우선합니다" 명시 후 감정 쪽으로 결론.

4단락 — 꿈이 가리키는 현실의 구체적 국면(재물·관계·건강·일·자기) 중 1가지를 단정 지목하고, 그 영역에서 어떤 변화가 임박했는지 1~2문장 묘사. 시점 단서(반복몽=장기 / 새벽몽=가까운 미래 / 한밤중몽=내면 메시지).

[동양식 안전장치]
- [역몽 규칙] 먼저 점검. 피·죽음·불·똥·무덤은 길몽 가능성 1순위.
- 상징 매칭이 없다면 단정적 길흉 판정 피하고 보수적으로.
- ※ 동양식 섹션에는 불릿이나 실천 처방·주의 사항을 두지 마세요. [advice]·[caution] 섹션에서 통합 처리합니다.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[서양식 작성 지침 — western_interpretation]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
총 1100~1500자. 프로이트 정신분석 + 융 분석심리학 + 융 동시성. 학문 용어를 한국어 부연과 함께.

1단락 — 첫 줄에 은유 제목 1줄 (동양식과 다른 표현). 다음 줄에 "정신분석·분석심리 관점에서 보면…" 도입.

2단락 — 프로이트 정신분석. 가능하면 다음 용어 중 1~2개를 한국어 풀이와 함께:
   · '에고 아이덜(Ego Ideal, 자아 이상)' — 인물·연예인 등장 시 이상형 투사
   · '리비도(Libido)' — 성·창조 에너지의 흐름
   · '억압된 욕망' — 의식이 거부해 무의식이 꿈으로 표출
   · '소망 성취(Wish Fulfillment)' — 현실에서 이루기 어려운 것을 꿈으로 충족
사용자 장면 인용하며 무의식 의미 풀이.

3단락 — 융 분석심리. 다음 용어 중 1~2개를 한국어 풀이와 함께:
   · '아니마(Anima, 남성 안의 여성성)' / '아니무스(Animus, 여성 안의 남성성)' — 이성 인물 등장
   · '그림자(Shadow)' — 의식이 외면한 자기 측면
   · '코니운크티오(Coniunctio, 대립의 통합)' — 성적·통합적 결합 장면
   · '페르소나(Persona)' — 사회적 가면

4단락 — 융 '동시성(Synchronicity)' 이론. 현실에서 어떤 사건·만남·기회·인정을 예고할 수 있는지. "무의식이 미래의 신호를 먼저 감지한 것일 수 있어요" 톤. 사주는 끌어오지 말 것.

5단락 — 경고와 통찰. '인플레이션(Inflation, 자아 팽창)' 또는 '투사(Projection)' 함정 중 1개 + 현실 도피 경고.

[서양식 안전장치]
- 학문 용어는 반드시 한국어 부연 병기.
- 한 단락에 같은 학파 용어 3개 이상 몰지 말 것.
- 성·연예인·금기 입력이라도 진지하게 다루되 욕망·자아·통합 같은 추상으로 승화.
- ※ 서양식 섹션에도 실천 처방·행동 권고·주의를 두지 마세요. [advice]·[caution] 섹션에서 통합.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[조언 작성 지침 — advice ("이렇게 하면 좋아요")]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
긍정 행동·실천 권고 본문 + 행운/액막이 항목 두 부분.

(1) 본문: 240~360자, 4~6문장. 자연 문단 (불릿·하이픈 금지). 톤은 격려·응원.
   - 앞으로 1주~1달 시점의 구체 실천 1~2개 (시점 명시).
   - 사용자 정황·인물·감정 인용 1회 이상.
   - 동양식·서양식 어휘와 다른 새 어휘.
   - 격려·응원 톤. "~해보세요", "~기회예요" 같이.

(2) 항목: 본문 다음에 빈 줄 1줄을 두고 4~6개 항목을 한 줄에 "키: 값" 형식.
   - 키 화이트리스트:
     [길몽일 때] 색, 방향, 시간, 숫자, 활동, 보석
     [흉몽일 때] 액막이, 환경, 조심할 시간, 조심할 방향, 조심할 색, 보호
     [혼재몽일 때] 위 두 묶음에서 4~6개 혼합
   - 값은 짧게 (10~30자, 2~3개 단위면 "A, B").
   - 한 줄 = 1 항목. 빈 줄·들여쓰기 금지.

예 (길몽):
적어주신 설렘과 기쁨은 ... (본문 자연 문단 4~6문장)

색: 청록, 은백
방향: 동, 동남
시간: 새벽~이른 아침
숫자: 3, 7
활동: 새로운 모임 참석, 작은 아이디어 메모

[조언 안전장치]
- 본문에 ":" 가 우연히 들어가지 않도록 주의 (값 추출 패턴과 충돌).
- 항목 줄에서 "키:" 는 반드시 화이트리스트 단어만.
- 주의·경고는 [caution] 으로 분리 — advice 본문에 두지 말 것.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[주의 작성 지침 — caution ("주의할 점")]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
총 160~260자, 3~5문장. 자연 문단 (불릿·하이픈 금지).

- 이 꿈이 시사하는 함정·놓치기 쉬운 부분·과신 위험.
- 피해야 할 행동·말·관계 1~2가지 (구체적으로).
- 사용자 정황·인물·감정 인용 1회 이상.
- 격려 톤이 아닌 차분한 경고 톤. 단 놀라게 하는 단정 표현 금지.
- 흉몽이면 "주의해야 할 시기·시점" 단정 가능. 길몽이면 "방심 금지·자만 경계" 톤.
- advice 본문에 이미 나온 행동을 반복하지 말 것 — 다른 관점.

예 (길몽):
다만 적어주신 강렬한 만족감에 휩쓸리다 보면 ... (3~5문장 차분한 경고)

예 (흉몽):
도망과 불안의 감정이 강했다고 적으신 만큼, 다음 한 주는 ... (구체적 회피 가이드)`;
};

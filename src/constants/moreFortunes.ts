...
export const MORE_FORTUNE_CONFIGS: Record<MoreFortuneId, MoreFortuneConfig> = {
  ...
  name: {
    ...
    // 섹션별 가이드 합산: 한글모드 2,300~3,000자 / 한자모드 2,600~3,400자.
    // (summary 480~620자 증량 반영 — 사용자 피드백 "종합평가 양 너무 적음")
    // fortuneService.buildNameFortunePrompt 에서 hanjaName 유무로 1.25배(한글) / 1.5배(한자) 가산.
    // 한자모드 본문 3,400자 + 마커 7개 + 4격 표시 + 은유줄 합산. Gemini 한국어
    // 토큰화 1자 ≈ 1.7토큰 가정 시 약 6,300토큰 → 안전 마진 두고 7,500.
    maxTokens: 7500,
    needsNameInput: true,
  },
  dream: {

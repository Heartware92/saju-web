...
export const MORE_FORTUNE_CONFIGS: Record<MoreFortuneId, MoreFortuneConfig> = {
  ...
  name: {
    ...
    // 섹션별 가이드 합산: 한글모드 2,300~3,000자 / 한자모드 2,900~3,800자.
    // (summary 480~620자 증량 + four_axis 5축 확장 + axis_eum 격국 인용 + axis_eumyang 신설)
    // fortuneService.buildNameFortunePrompt 에서 hanjaName 유무로 1.25배(한글) / 1.5배(한자) 가산.
    // 한자모드 본문 3,800자 + 마커 8개 + 4격 표시 + 은유줄 합산. Gemini 한국어
    // 토큰화 1자 ≈ 1.7토큰 가정 시 약 7,000토큰 → 안전 마진 두고 8,200.
    maxTokens: 8200,
    needsNameInput: true,
  },
  dream: {

 ...
 * "더 많은 운세" 카테고리 메타 정보
 * - 홈 탭 아래 버튼으로 노출
 * - 각 카테고리는 달(moon) 크레딧 5개 소모 (2026-05-16 단일 달 크레딧 통합 후 단가)
 * - 기본 풀이 7종(정통/신년/궁합/지정일/택일/토정/자미두수, 10개 소모)과 달리 짧고 집중된 풀이
 *
 * [B안 적용 — 2026-04-27]
...
export const MORE_FORTUNE_CONFIGS: Record<MoreFortuneId, MoreFortuneConfig> = {
  ...
  name: {
    ...
    // 7섹션 풀버전 — 5달 운세 동급 분량 (자녀·출산운 2000~2600자 기준 + 5달 가치 유지).
    // 섹션별 가이드 합산: 한글모드 2,300~3,000자 / 한자모드 2,600~3,500자.
    // (summary 480~620자 증량 + axis_eum 격국 정형구 인용 반영)
    // [BACKLOG] 5축(수리 음양 포함) 부활 시 한자모드 3,800자 + maxTokens 8,200 으로 증량
    // fortuneService.buildNameFortunePrompt 에서 hanjaName 유무로 1.25배(한글) / 1.5배(한자) 가산.
    // 한자모드 본문 3,500자 + 마커 7개 + 4격 표시 + 은유줄 합산. Gemini 한국어
    // 토큰화 1자 ≈ 1.7토큰 가정 시 약 6,500토큰 → 안전 마진 두고 7,800.
    maxTokens: 7800,
    needsNameInput: true,
  },
  dream: {
...
// 2026-05-16 단일 달 크레딧 통합 — 더 많은 운세 단가 1 → 5 로 변경
// (creditCosts.ts 의 MOON_COST_MORE 와 동기화. 한 곳만 바꾸면 모든 화면 자동 반영)
export const MOON_COST_PER_FORTUNE = 5;

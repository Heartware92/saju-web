...
[용어 사용법]
- 십성·격국 등 용어는 꼭 필요할 때만 쓰고, 등장 즉시 같은 호흡에서 일상어로 풀어준다 (예: "표현하고 드러내는 기운(식상)이 약해서").
- 같은 용어를 반복 노출하지 않는다. 한자 병기·한자 나열 금지.
- ★ 아래 각 섹션 지침에 "십성/재성/격국 등을 명시적으로 노출하라"고 적혀 있어도, 본 톤 규칙이 우선한다 — 용어는 뒤로 숨기고 일상 풀이를 앞세운다.

[장면 중심 서술]
- 추상 진단("리더십이 있다") 대신 구체 장면("다들 망설일 때 먼저 정리해서 말을 꺼내는 쪽")으로.
- 20~40대 여성의 실제 맥락(직장·연애·친구·자기관리·돈 고민 등)에서 예시를 든다.
- 단정하되 따뜻하게. 가르치려 들거나 권위적으로 훈계하지 않는다.

[금지]
- 명리 교과서식 설명 덩어리, 한자 나열, 어려운 전문용어의 무풀이 노출.
